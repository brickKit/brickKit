...
所以配置里有它、却不启动，是**正常现象**。`up` 会如实说明理由：
...
#### 契约文件（`.proto` / `openapi.json`）怎么办

同项目内的组件，`brickkit add` 会把它的 artifacts 下到
`.brickkit/artifacts/<版本化服务名>/`（§7.1），**默认跟着项目一起提交**（§11）。

跨项目的服务没有这条路，而且不该有：那个组件不在你的 `components` 里，
平台也不知道该去哪个安装源找它——它甚至可能根本没发布到你这边的市场。
**契约由对方主动给一份**（第 0 步谈定的三件事之一）。

**放哪里：** 放进你自己的仓库，并沿用平台那套目录形状：

```
contracts/
  infra-notifier-1-0-0/          ← 版本化服务名，与平台的 artifacts 目录同一种命名
    api-contract/
...
**为什么要沿用版本化服务名：** 平台对同项目内组件用它，是为了让
"同一个组件的两个版本"各占一个目录、不会互相覆盖。跨项目更需要这个——
平台在这里**不再替你检查版本**，目录名就成了唯一记着"我按 1.0.0 写的客户端"的地方。
对方升到 1.1.0 时你新建一个目录、逐步切过去，`git diff` 里看得见；
就地覆盖的话，客户端代码悄悄对着新契约重新生成，没有任何一处提示。

**提交它。** 这不是缓存，是你这个项目的一部分依赖信息，
和 vendor 一份第三方 SDK 是同一件事。
