...
components:
  - id: people/basic          # 组件 ID（必须）
    ...
    enabled: true             # 可选。不写 / true 都是启动；只有 false 不启动
...
| `enabled: true` | ✅ 启动（与不写完全等价，只是把默认值写了出来） |
| `enabled: false` | ❌ 不启动 |

一句话：**配置里有它、又没写 `enabled: false`，就启动。**

这与 012 §2.3 的"brickkit.yaml 就是声明"是同一条：写了几个版本就启动几个容器，
写了哪些组件就启动哪些组件。

#### 强依赖与弱依赖在这件事上一视同仁

弱依赖（`optional: true`）写进配置之后照常启动。`optional` 管的是另外两件事，
都不在这一层：
...
> **这里曾经有第三件事：「就算装了也别自动启动」。已删除。**
>
> 它与"写了就启动"直接冲突，造出了"配置里有它、`up` 却不起它"这种要专门
> 写一节去解释的现象——而 `brickkit add` 恰恰会把整棵依赖树（含弱依赖）
> 写进配置。
>
> 当初的理由是"自动启动等于把**可选**悄悄变成**必启**"。这句话站不住：
> 起来了也不等于必启，调用方照样会在它挂掉时降级。而真正的代价是**发现性**——
> 一堆弱依赖默认关着，使用者装了组件却发现一半功能是哑的，
> 没有任何一处告诉他为什么；他得先知道"弱依赖不会自动启动"这条规则，
> 才想得到去翻配置。
>
> 反过来（默认全开、嫌重再关）失败方式温和得多：多几个容器，
> `docker ps` 里看得见，写一行 `enabled: false` 就没了。
> `up --dry-run` 还会专门列出"只被弱依赖引用"的那些——那就是可以下手的名单。

#### 关掉一个组件不会连累别人

关掉 A 之后，只有 A 不跑。**没有人会被顺带关掉**，哪怕它只有 A 一个依赖方。
...
components:
  - id: erp/backend
    ...
    enabled: false     # ← 只有它不跑
  - id: people/basic
    version: 1.0.0     # ← 照常跑，哪怕现在没人调它
```

> **这里曾经有一套「级联」：关掉一个组件，依赖链上无人需要的组件自动跟着跳过。
> 已删除。**
>
> 它在做一次**隐式**决定，而平台的原则是"精确优于隐式"：使用者关的是 A，
> 平台顺手把 B、C 也停了，输出里那几行"级联跳过"很容易被当成正常现象略过去；
> 等他发现 B 没跑，线索已经断了。
>
> 它还带来一整套要背的规则——三态 `enabled`、"钉住"、"根组件"判定、
> "只被弱依赖引用的不会被自动拉起"——每一条都要单独辩护一遍。
>
> 想收窄这次启动的范围，用 `brickkit up --only <组件>`（004 §3.5）：
> **显式说要哪几个**，而不是关掉一个再让平台倒推还剩哪几个。
> 它也不动配置，不会在团队共享的 `brickkit.yaml` 上留下个人的临时偏好。

#### 强依赖被关掉时报错

如果某个**正在启动**的组件强依赖了一个被 `enabled: false` 关掉的组件，
`up` 报错并点名依赖方：
...
❌ 错误：强依赖被显式关闭
   ...
   原因：依赖方启动后必然连不上它；平台不会替你把依赖方一起关掉——那是一次隐式
        决定，而它们没跑的线索只有输出里一闪而过的一行
   建议：
   1. 要一起关掉就给依赖方也写 enabled: false
   2. 或者移除 erp/backend 的 enabled: false
   3. 只是这一次想跑一个子集的话，用 brickkit up --only <组件>，不必动配置
```

一次报出**全部**冲突：关掉一个底层组件往往同时影响好几个依赖方，
一次只说一个会让人"改一行、跑一次"来回好几趟。

**弱依赖不在此列**：关掉一个只被弱依赖引用的组件是完全正常的用法，
调用方拿不到 `*_ENDPOINT`，按 002 §3.4 自行降级，`up` 只警告一句。
...
components:
  ...
  - id: portal/user-frontend
    version: 1.0.0
    enabled: false         # ← 它强依赖 erp/backend，必须一起关，否则报错
```
...
- `brickkit add` 自动添加的组件不写 `enabled` 字段——不写就是启动，没什么要声明的
- 只有 `enabled: false` 才改变行为
- 关掉一个被强依赖的组件会报错，不会静默连累依赖方
...
只会在某条命令上表现成一个奇怪的行为——`down --only` 与 `status` 都真的踩过。
...
| 写法 | 平台的行为 |
| --- | --- |
| `enabled: false` | 只有它自己不跑；有组件强依赖它时 `up` 报错并点名依赖方 |
...
components:
  # 部门组件：正常 Docker 模式（没写 enabled → 启动）
  - id: department/tree
    version: 1.0.0
...
| 8 | 暂时不用的组件写 `enabled: false` 而不是删掉 | 保留声明，随时能开回来；`brickkit sync` 也会把它的源码收进 `.archived/` |
...
| 强依赖被关掉         | ❌ 报错并点名依赖方——平台不会替你把依赖方一起关掉（§4.3）                                                                 |
