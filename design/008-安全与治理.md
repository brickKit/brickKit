...
>
> 市场若要做有意义的校验，前提是**发布者公钥在账号下登记并有独立的变更审计**，
> 那是另一件事（见《开发进度》延后清单 P30）。
>
> 市场也**不强制**签名：强制与否是使用者的策略（§8.5），同一个市场同时服务着
> "本地开发随便装"和"生产必须验签"的项目，在发布侧一刀切等于替所有人做了决定。
...
签名覆盖的是**组件版本的 Manifest**，不是镜像。镜像签名存在 registry 里、由集群的准入控制器校验，CLI 碰不到它；Manifest 才是 CLI 真正下载、真正能验的东西（镜像签名另行规划）。
...
组件在 Manifest 中声明自己需要的资源和依赖。安装时，管理员可以看到组件需要什么，确认后安装。

### 9.2 组件 Manifest 中的权限相关声明
...
管理员执行 `brickkit add` 时，CLI 展示：

```
📦 添加 people/basic@1.0.0
   该组件需要：
   📦 依赖组件：
      - department/tree@1.0.0（未添加，将自动添加）
   🗄️ 资源需求：
      - PostgreSQL 数据库（请在 brickkit.yaml 中声明并绑定）
   📊 推荐资源配额：
      - CPU: 100m ~ 500m
      - 内存: 128Mi ~ 512Mi
   📁 产物文件：
...
| 组件只能访问自己的数据库 | 不共享表 |
| 组件不获取其他组件的密钥 | 密钥隔离 |
| 组件不获取管理员权限 | 组件不需要管理权限 |
| 组件不能覆盖平台保留变量 | config 配置项名称不得与保留变量冲突 |
...
| 信任级别 | 说明 | 隔离方式 |
| --- | --- | --- |
| 官方组件 | BrickKit 官方发布 | 标准容器 |
| 可信社区组件 | 社区发布，签名校验通过 | 标准容器 |
| 未验证组件 | 无签名或签名未知 | 标准容器 + 提示风险 |
| 被阻止组件 | 被标记为 blocked | 不允许安装 |

**信任模型补充：** 上述信任级别仅影响 CLI 的提示和安装阻断行为，不代表平台对组件代码安全性的审查或背书。安装任何组件（包括"可信社区组件"）都意味着用户信任该组件的发布者。详见 1.4 节"BrickKit 组件信任模型"。
...
| 002 组件规范 | 定义组件权限声明；破坏性变更（Major）协同守则；deployment.resources 字段定义；configSchema 保留变量约束；健康检查禁令；artifacts 字段定义；API 契约公开原则 |
