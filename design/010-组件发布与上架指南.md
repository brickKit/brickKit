...
| 11 | observability 字段（可选） | 是否暴露 metrics/tracing |
| 12 | 破坏性变更已标注（Major 版本） | Changelog 中强提示（见 10.7） |
| 13 | artifacts 声明完整 | 如果组件提供 API，必须声明 API 契约文件 |
| 14 | 闭源组件包含 API 契约 | 闭源组件的 artifacts 中必须有至少一个 `type: api-contract` |
...
# 对镜像签名
cosign sign --key cosign.key registry.brickkit.io/people-basic:1.2.0
```

### 6.3 签名信息

签名后，你需要记录以下信息用于上传到市场：
...
```bash
cosign verify --key cosign.pub registry.brickkit.io/people-basic:1.2.0
```
...
如果组件在 brickkit.yaml 中恢复启用（级联计算后会启动），执行 `brickkit sync` 会自动将其从归档目录移回活跃目录：
