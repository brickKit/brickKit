...
components:
  - id: hello/world
    version: 1.0.0
    # 没写 enabled → 启动（只有 enabled: false 才不启动）
```
...
当项目中有很多组件源码时，你日常开发可能只关注其中 2~3 个组件。`brickkit sync` 可以把不活跃的组件源码归档到 `components/.archived/` 目录（`--only` 还能只留下你点名的那几个及其依赖）：
...
**核心规则：**
- `brickkit sync` 与 `brickkit up` 共用同一套启停判定
- 双向操作：该归档的归档，该激活的移回来
...
写上 `enabled: false`。不写、或写 `enabled: true` 都是启动。
...
关掉一个组件只有它自己不跑，不会连累别人。但如果有**正在启动的组件强依赖它**，
CLI 会报错并点名依赖方——要一起关就给它们也写上 `enabled: false`（003 §4.3）。
弱依赖不在此列：调用方拿不到 `*_ENDPOINT`，按自己的降级逻辑处理即可。
...
| 工作区管理         | `brickkit sync` 双向归档/激活组件源码。归档目录为 `components/.archived/`                                          |
