...
**所有资源一律探。** 平台不部署它们（§10.1），它们本该在 `up` 之前就已经跑着。
从前这里还要先排除"由 CLI 托管的那些"（它们正要靠这次启动带起来，探它必然失败）——
托管取消之后这条例外也没有了。

端口检查会排除本项目自己的容器，否则第二次 `up` 会把上一次留下的自家容器
报成"端口被占用"。
...
| 维度            | 说明                                     |
| ------------- | -------------------------------------- |
| 资源            | 运维/使用者部署 + brickkit.yaml 声明 + 可选健康检查。**平台不生成资源容器** |
