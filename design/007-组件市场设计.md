...
| --- | --- |
| draft → stable | 组件发布者 |
| stable → deprecated | 组件发布者 |
| 任意 → blocked | 市场管理员（发现安全问题时） |
...
| POST | /api/v1/components | 创建组件（首次发布） |
| PUT | /api/v1/components/{componentId} | 更新组件元数据 |
| DELETE | /api/v1/components/{componentId} | 删除组件（仅所有者） |
...
| GET | /api/v1/components/{id}/versions/{ver} | 查询版本详情 |
...
| POST | /api/v1/components/{id}/versions/{ver}/artifacts | 上传产物（发布时使用） |
...
| 命名空间 | 谁能发布 |
| --- | --- |
| brickkit/ | 只有 BrickKit 官方团队 |
| infra/ | BrickKit 官方或授权组织 |
| 组织名/ | 该组织的成员 |
| 用户名/ | 该用户 |
| 业务域/ | 该业务域的所有者 |
...
组件搜索通过市场 Web 界面或直接调用市场 API（`GET /api/v1/components?keyword=xxx`）完成。CLI 不提供搜索命令。

用户可以在市场 Web 界面中按关键字、标签、命名空间、版本状态、可见性、来源类型等维度搜索和浏览组件。
...
- 发布方式：CLI `brickkit publish` + 市场 Web 界面。
...
- 组件搜索通过市场 Web 界面或直接调用市场 API 完成。CLI 不提供搜索命令。
...
| 组件搜索         | 通过市场 Web 界面或直接调用市场 API 完成，CLI 不提供搜索命令                                    |