...
`--remove-orphans` 删的是"compose 文件里没有的容器"，而 `--only` 生成的部署文件
**只含被点名的那个子集**——其余组件全部落进它的射程。
无条件带上它，一条 `brickkit up --only demo/hello` 就会把另外几个正在服务的组件
连同 postgres 容器一起**删掉**（不是停止，是 `rm`），而 `up` 报的是成功。

那正是上表第三条论证过不可接受的事。因此两种目标共用**同一个判据**：

| 本次是 | K8s | Docker |
| --- | --- | --- |
| 整个项目一起起 | 按项目标签比对并清理 | 带 `--remove-orphans` |
| `--only` 只起子集 | **完全不清理** | **不带 `--remove-orphans`** |

判据只有一份（`pruneSelectorFor`），两边就不会再分叉。早先 Docker 侧无条件带着它，
理由写的是"compose 的 `--remove-orphans` 兜底"——那句话把"清理孤儿"与
"删掉没点名的"当成了同一件事，而它们正好相反。
...
> ⚠️ PDB 是**第一个条件生成的资源**（其余每种资源都随组件必然生成）。
> §5.9.1 的孤儿清理因此不能只比对名字——PDB 与 Deployment 同名，
> 副本数从 3 改回 1 之后那份 PDB 会被当成"该留的"，从此永远拦着 drain。
> 这个 bug 在 minikube 上真跑到过（P35）。
...
以及三种形态各自的代价——`external`（003 §4.9）只是其中最贵的那一种。
...
### 5A.4 形态三：外部依赖（`external`）

只有当**组件本身就是权威**时才走这条：单例副作用（定时任务、通知发送——跑两份就是
双倍邮件）、成本高昂（大模型推理服务）、归属独立（一个团队运维，别人只许调用）。

字段语义、互斥规则与失败模式见 003 §4.9，这里只补两条部署侧的事实：

| 目标 | 跨项目寻址 | 平台的动作 | 对方没部署时 |
| --- | --- | --- | --- |
| Docker | 服务名**只在同一张网络**里解析 | 把**依赖它的**服务接进对方项目的网络（只引用不创建） | `up` **当场失败**，点名缺哪张网络、该去哪个项目 up |
| K8s | 跨命名空间 DNS 原生可用 | 注入的地址带上后缀：`<服务名>.<对方命名空间>` | 本项目正常起来，调用时才失败 |

Docker 侧只接依赖方、不是全部接进去：网络是可达性边界，全接进去等于把
"谁能访问共享服务"从**按依赖声明**变成**默认全通**。

Docker 那边**当场失败是更好的结果**：那时依赖方连地址都解析不了，
先起来只会把问题推迟到运行时。CLI 因此在 `up` 前先查一次那张网络
（003 §4.9）——compose 自己报的那句话里没有 external、没有项目名、也没有下一步。

K8s 侧**不生成同名 Service** 尤其要紧：本命名空间里的同名 Service 会**抢在**
跨命名空间解析之前命中，而它背后一个 Pod 都没有——表现是稳定的 503，
