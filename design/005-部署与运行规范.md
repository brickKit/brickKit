...
Docker 那条路由 compose 的 `--remove-orphans` 兜底；K8s 这边由 CLI 自己做：
...
| **`--only` 时完全不清理** | 那时只部署子集，没被点名的组件照样在正常服务。把它们当孤儿删掉是**把线上组件下线**，比留个孤儿严重得多 |
| **删掉什么必须打印出来** | 使用者可能只是误删了 `brickkit.yaml` 里的一行，本意并非下线那个组件——那几行输出是他唯一的线索 |

清理范围是 Deployment / Service / Ingress / NetworkPolicy / ServiceAccount / Job。
**刻意不含 Namespace**：它上面也有项目标签，一旦算进去，一次普通的升级
就会把整个命名空间连同里面所有东西删掉。也不含 Secret：它按资源 ID 命名，
与"本次部署的服务名集合"对不上。
