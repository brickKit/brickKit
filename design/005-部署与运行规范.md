...
# 等价于：
docker compose -f .brickkit/generated/docker-compose.yaml up -d

# 查看状态
brickkit status

# 等价于：
docker compose -f .brickkit/generated/docker-compose.yaml ps

# 一键停止
brickkit down

# 等价于：
docker compose -f .brickkit/generated/docker-compose.yaml down
# 注意：不删除 volume（数据安全）。如需彻底清理，请手动执行 docker volume rm

# 查看日志
docker compose -f .brickkit/generated/docker-compose.yaml logs -f erp-backend-1-0-0
```
...
| --- | --- |
| type: http, path: /healthz | healthcheck: test: ["CMD", "wget", "-q", "--spider", "http://localhost:8080/healthz"] |
...
Podman 中使用 `host.containers.internal` 替代。
...
**组件代码零修改。** 因为它读的是环境变量，端口是多少由 CLI 注入决定。
...
| 用户未指定 `localPort` | CLI 自动分配（从 8081 开始递增） |
| 端口冲突 | CLI 报错并提示更换端口 |
...
| 依赖组件没有 `expose` | CLI 自动分配宿主机端口（从 18080 开始递增） |
| 端口冲突 | CLI 报错并提示 |
...
metadata:
  ...
  namespace: brickkit-my-project
  labels:
    brickkit.io/component-id: portal/user-frontend
spec:
  rules:
...
type: Opaque
stringData:
  password: ${PEOPLE_DB_PASSWORD}   # 从 .env 或环境变量获取
---
apiVersion: v1
kind: Secret
metadata:
...
# 等价于：
kubectl apply -f .brickkit/generated/k8s/namespace.yaml
kubectl apply -f .brickkit/generated/k8s/secrets/
kubectl delete job/people-basic-1-0-0-migration --ignore-not-found  # 清理旧 Job
kubectl apply -f .brickkit/generated/k8s/migrations/       # 先执行迁移
kubectl wait --for=condition=complete job/people-basic-1-0-0-migration  # 等待迁移完成
kubectl apply -f .brickkit/generated/k8s/deployments/
kubectl apply -f .brickkit/generated/k8s/services/
kubectl apply -f .brickkit/generated/k8s/ingress/

# 查看状态
brickkit status

# 等价于：
kubectl get pods -n brickkit-my-project
kubectl get services -n brickkit-my-project

# 一键删除
brickkit down

# 等价于：
kubectl delete -f .brickkit/generated/k8s/
```
...
| host 映射 | host-gateway | host.containers.internal |
...
| Podman | people-basic-1-0-0:host.containers.internal |

CLI 根据检测到的引擎自动选择正确的值。
...
# 查看最近100行
docker compose -f .brickkit/generated/docker-compose.yaml logs --tail 100 people-basic-1-0-0
```
