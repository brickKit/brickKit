...
**`--only` 点到没有容器的组件时（`local: true` / `external`），说明原因并跳过。**
它们在依赖图里、在 `status` 里也看得见，但部署文件里没有对应的 service——
...
**`external` 的组件同理单列一节**（003 §4.9）。混进"未在运行"那张表的话，
它会变成一条**常驻的假失败**：本项目从来就不为它生成容器，引擎里当然查不到，
而使用者看到红叉会去 `docker ps` 里找它、找不到、然后开始怀疑平台。

这里只**陈述**它由哪个项目部署，不去核实对方到底部没部署——那需要读另一个项目的
部署文件，而"平台替你探一下"这条路已经在 006 §8 上论证过。`up` 会在 Docker 目标下
先查一次对方的网络在不在（003 §4.9），那是**启动前的阻断**，与这里的"报告现状"
是两回事。
