...
| `brickkit sync` | 整理源码工作区 | 根据级联计算结果，双向归档/激活组件源码 |
...
4. 清理对应的 Manifest 缓存
5. 清理 `.brickkit/artifacts/` 中对应组件的产物缓存目录
6. 自动删除 `components/<scope>/<name>/` 源码目录（如果存在）
...
2. 它是根组件（没有任何组件依赖它）且未被显式关闭
3. 至少有一个正在启动的组件强依赖它，且它未被显式关闭

一个组件被级联跳过，当且仅当：

1. 它没有写 `enabled` 字段（不是钉住的）
2. 它不是根组件
3. 没有任何正在启动的组件依赖它
...
🚀 启动项目 my-project（deploy.target: docker）
 ...
 🔧 执行数据库迁移：erp-backend（python manage.py migrate）✅
 🐳 正在启动...
 ✅ 全部组件已启动
    department-tree-1-0-0       running
    people-basic-1-0-0          running
    auth-rbac-1-0-0             running
    erp-backend-1-0-0           running
    portal-user-frontend-1-0-0  running
 💡 查看状态：brickkit status
 💡 查看日志：docker compose -f .brickkit/generated/docker-compose.yaml logs -f
 💡 本地调试：在 IDE 中加载 .brickkit/generated/local-debug.people-basic-1-0-0.env 启动 people/basic
```

### 3.5.1 升级时的额外行为

当 `brickkit up` 检测到 brickkit.yaml 中某组件的版本号与本地缓存的 Manifest 版本不一致时（即用户修改了版本号进行升级），CLI 执行以下额外步骤：
...
- Docker：`docker compose -f .brickkit/generated/docker-compose.yaml down`
- K8s：`kubectl delete -f .brickkit/generated/k8s/`

**停止顺序：** 与启动顺序相反（依赖方先停，被依赖方后停）。
...
📊 项目状态：my-project（deploy.target: docker）
 ...
 │ authorization/rbac        │ 1.0.0   │ ● 运行中 │ 8080     │
 │ erp/backend               │ 1.0.0   │ ● 运行中 │ 8080     │
 │ portal/user-frontend      │ 1.0.0   │ ● 运行中 │ 80       │
 └──────────────────────────┴─────────┴─────────┴──────────┘
 ⬜ 已禁用 / 级联跳过（2个组件）
 ┌──────────────────────────┬─────────┬──────────────────────────────┐
 │ 组件                      │ 版本    │ 原因                          │
 ├──────────────────────────┼─────────┼──────────────────────────────┤
 │ infra/redis-event-bus     │ 1.0.0   │ 显式禁用                      │
 │ portal/admin-frontend     │ 1.0.0   │ 级联跳过（依赖 erp/backend）  │
 └──────────────────────────┴─────────┴──────────────────────────────┘
 本地调试组件：
    people/basic → localhost:8081（IDE 调试模式）
 资源状态：
    postgres-main    ● 可达
    redis-main       ● 可达
```
...
**用途：** 根据级联计算结果，整理组件源码工作区。将不启动的组件源码归档到 `components/.archived/`，将需要启动的组件源码从归档目录移回 `components/`。
...
flowchart TD
    ...
    C & D & E & F & R & G & H & J & K & L & M --> N["输出 docker-compose.yaml"]
```
...
| Podman | `podman-compose -f <file> ps` |
...
❌ 错误：数据库迁移失败
   ...
   原因：迁移脚本执行失败，主服务未启动
   建议：
   1. 查看迁移日志：docker compose -f .brickkit/generated/docker-compose.yaml logs people-basic-1-0-0-migration
...
| 依赖 | 必须 | 说明 |
| --- | --- | --- |
| Docker 或 Podman | ✅（deploy.target: docker） | 本地部署 |
| docker compose / podman-compose | ✅（deploy.target: docker） | 编排 |
