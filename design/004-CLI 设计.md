...
必须最后启动：people/basic（需等前 4 个组件就绪）
...
> 这一块与上面几处输出一样，由 `make lint` 的 `check-guide-output` 拿**真实 CLI 输出**逐行比对。
> 它从前是手写的：服务名 `auth-rbac-1-0-0` 拼错（应为 `authorization-rbac-1-0-0`）、
> "可独立启动"那行打印的是序号而不是服务名、"必须最后启动"的括号里写的也不是 CLI 的说法。
