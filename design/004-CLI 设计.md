...
| 点名的组件只被弱依赖引用 | ✅ 照样启动——点名就是最明确的意图（见下） |
| 点名的组件的**强依赖**是 `enabled: false` | ❌ 报错阻断（两个意图直接冲突） |
| `--only` 指定的组件本身是 `enabled: false` | ❌ 报错阻断 |
| 没被点名的根组件 | ⬜ 不启动——收窄掉它们正是 `--only` 的意思 |

**点名等于钉住。**

级联回答的是"**你没说的时候**该跑什么"（003 §4.3）。你在命令行上点了名，
那就是最明确的意图，与 `enabled: true` 同级——所以 `--only` 是在**重新计算**，
不是在级联结果里再挑几个。

这个区别有实际后果。只被弱依赖引用的组件默认不启动（003 §4.3），
如果拿级联结果做交集，`brickkit up --only infra/redis-event-bus` 会**什么都不启动**：

```
🎯 --only：只启动 infra/redis-event-bus 及其强依赖
   ⬜ infra/redis-event-bus@1.0.0  级联跳过（只被弱依赖引用…）
📋 本次没有组件会启动
```

——而这一节承诺的是"只启动指定组件及其依赖"。实现曾经就是这样（已修）。

把点名当成钉住之后，另一件事自动正确了：点名的组件**强依赖被关掉**时会报错，
与钉住的组件走同一条路，而不是静默启动一个必然连不上依赖的组件。
...
同一份级联结果。多出来的差别只有三处，没有一处值得再养一个命令：
...
| brickkit up --only         | 支持 `@版本` 写法。**点名等于钉住**：只被弱依赖引用的组件照样启动，根组件不再自动启动；点名的组件本身或它的强依赖是 `enabled: false` 时报错 |
