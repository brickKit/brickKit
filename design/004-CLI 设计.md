...
| brickkit.yaml 就是声明 | 写了几个版本就启动几个容器；写了哪些组件就启动哪些组件；只有 `enabled: false` 不启动 |
...
| # | 职责 | 说明 |
| --- | --- | --- |
| 1 | 管理项目配置 | 维护 brickkit.yaml：组件列表、启用/禁用、本地调试、暴露声明、资源绑定、配置覆盖、资源配额覆盖、部署目标 |
...
| 6 | 管理组件源码工作区 | `brickkit add --repo` / `--repo-all` clone 组件源码；`brickkit sync` 把用不上的组件源码归档/激活；`brickkit remove` 自动删除源码目录 |
...
**关键规则：** `brickkit add` 自动添加的组件不写 `enabled` 字段——不写就是启动，没什么要声明的（003 §4.3）。
...
强依赖与弱依赖一视同仁：`optional: true` 管的是"取不到算不算错"（解析期）
与"没跑时注不注入 `*_ENDPOINT`"，不管"跑不跑"。

关掉一个组件只有它自己不跑；如果有**正在启动**的组件强依赖它，`up` 报错并点名依赖方
（弱依赖不在此列，那本来就是"有就用、没有就降级"）。
...
| 闭包里的组件 | ✅ 跟着启动——强弱依赖都算（见下） |
| 闭包里有 `enabled: false` 的**弱**依赖 | ⬜ 不启动，`enabled: false` 是长期声明，比这一次的点名更强 |
| 闭包里有 `enabled: false` 的**强**依赖 | ❌ 报错阻断（两个意图直接冲突） |
| `--only` 指定的组件本身是 `enabled: false` | ❌ 报错阻断 |
| 没被点名的组件 | ⬜ 不启动——收窄掉它们正是 `--only` 的意思 |

**闭包强弱都算。** 既然"跑不跑"上两者一视同仁，收窄范围时也该一致：
只取强依赖的话，`up --only erp/backend` 会得到一个**异步功能是哑的** erp/backend，
而使用者点它的名是想把它跑起来用，不是跑起来看。
...
❌ 无法启动指定的组件：
   ...
   如需启动，请先在 brickkit.yaml 中移除 enabled: false。
```
...
**`--only` 点到没有容器的组件时（`local: true`），说明原因并跳过。**
它在依赖图里、在 `status` 里也看得见，但部署文件里没有对应的 service——
把那个服务名递给引擎换来的是 `no such service`，而且会让**整条命令失败**。

⚠️ 点到的组件**全都**没有容器时，不能就这么把空名单交给引擎：空的服务名列表
在引擎眼里是**"停整个项目"**。那时 CLI 什么都不做，只说清楚为什么。
...
同一份启停判定。多出来的差别只有三处，没有一处值得再养一个命令：
...
**它真正不可替代的地方，是"留哪些"由平台算，而不是由你列。** `--only people/basic`
之后留下的不止 `people/basic`，还有它的**传递强依赖闭包**——隔着两三层的那些，
你自己列不出来。IDE 的"排除目录"能藏，藏什么却得你自己想清楚；
组件多到需要它的时候，恰好也是你想不清楚的时候。
...
组件被归档之后，它在 `brickkit.yaml` 里那一行**还在**——启停判定要判断它是否启动，
...
| 003 项目配置规范 | 定义 brickkit.yaml 完整格式（含 config 覆盖、exposePort、resources 覆盖）；enabled 的启停语义；local 组件依赖端口映射规则；local-debug.env；components/.archived/ 归档目录 |
...
| enabled                    | 不写 / `enabled: true` 都是启动；只有 `enabled: false` 不启动。强弱依赖一视同仁                                     |
| 强依赖被关掉                     | ❌ 报错并点名依赖方——平台不会替你把依赖方一起关掉（003 §4.3）                                                          |
...
| brickkit sync              | 双向归档/激活组件源码。不集成在 `brickkit up` 中，纯独立命令                                                       |
| brickkit remove            | 从 brickkit.yaml 移除 + 清理缓存 + **自动删除源码目录**                                                    |
| brickkit up --only         | 支持 `@版本` 写法。只启动点名的组件**及其依赖闭包**（强弱都算）；点名的组件本身或它的强依赖是 `enabled: false` 时报错          |
