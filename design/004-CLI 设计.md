...
| `--dry-run` | 只生成部署文件，不启动 |
| `--check-resources` | 启动前检查资源可达性与宿主机端口占用（**只警告，不阻断**） |
...
**`--check-resources` 检查什么：**

| 检查项 | 范围 | 说明 |
| --- | --- | --- |
| 资源可达性 | **只探外部资源**（IP / 域名） | CLI 托管的资源正要靠这次启动带起来，探它必然失败——一条永远出现的警告只会让人以后忽略所有警告 |
| 宿主机端口占用 | 本次要映射的全部宿主机端口 | 生成阶段只能保证项目内部不打架；这台机器上**别的进程**占着某个端口，得真的探一下才知道 |

端口检查会**排除本项目自己的容器**：否则第二次 `up` 会把上一次留下的自家容器报成"端口被占用"。

与 `--dry-run` 同用时照样体检，且不要求引擎可用——`--dry-run` 的意思是"告诉我会发生什么"，不是"什么都别做"。
...
⬜ 未启动（2 个组件）
   ...
   portal/admin-frontend    1.0.0    级联跳过（没有正在启动的组件强依赖它）
...
📦 资源状态
   ...
   redis-external           ○ 不可达（10.0.0.9:6379：connection refused）
```

**排版说明：** 输出用缩进对齐而不是框线表格。表头是中文、内容是 ASCII，框线要对齐得算东亚字符宽度，在不同终端下很脆；缩进风格也与 `up` / `order` 一致。信息一条不少。

**资源可达性怎么判（两类资源完全不同）：**

| 资源类型 | 判据 | 为什么 |
| --- | --- | --- |
| CLI 托管（`host` 是服务名） | 看该资源容器的状态 | 它在容器网络里，宿主机上 `dial postgres:5432` 连名字都解析不了 |
| 外部（IP / 域名） | 真的 TCP 拨一次号 | 运维已部署，只有连一下才知道通不通 |

判据必须与生成部署文件时"哪些资源由 CLI 托管"用**同一条**（006 §10.4），两处各判一次迟早会分叉。

**几个不出现在组件列表里的东西：** 迁移容器（`-migration`）与基础资源容器是平台的实现细节；使用者装的是组件，看到的也该是组件，资源单列一节。
