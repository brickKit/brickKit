...
| 6 | 管理组件源码工作区 | `brickkit add --repo` / `--repo-all` clone 组件源码；`brickkit sync` 把用不上的组件源码收进 `components/.archived/`（`--only` 按点名收拢，不改 brickkit.yaml）；`brickkit remove` 自动删除源码目录（含已归档的那一份） |
...
| `brickkit sync` | 整理组件源码工作区：把用不上的组件源码收进 `components/.archived/`；`--only` 只保留指定组件及其强依赖，且不改 brickkit.yaml |
