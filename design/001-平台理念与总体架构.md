...
| 环境一致 | 本地（Docker/Podman）与生产（K8s）使用同一套地址格式，组件代码零修改 |
...
组件代码不感知自己跑在 Docker、Podman 还是 K8s 上。
...
- 本地：Docker Compose / Podman 的 service name 就是 DNS 名
...
| 环境 | 地址格式 | 示例 |
| --- | --- | --- |
| 本地 Docker/Podman | `http://<服务名>:<端口>` | `http://people-basic-1-0-0:8080` |
...
```mermaid
flowchart LR
    subgraph Local["本地（Docker / Podman）"]
...
| 环境无关 | 组件代码不感知 Docker / Podman / K8s |
...
- ✅ Podman（podman-compose）与 Docker 体验一致
