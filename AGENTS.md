...
brickkit up --log-level off                       # every command has this flag: silence the JSON diagnostic log lines on stderr, the command's own result is untouched
BRICKKIT_LOG_LEVEL=debug brickkit up              # env var sets the default for a whole shell session instead of typing the flag every time
```
...
| A user says every command prints noisy `{"time":...,"level":"INFO",...}` lines they don't want to see | Those are structured diagnostic logs the CLI always writes to stderr (`--log-level`, default `info`) — a command's actual result (the human-readable stdout, the ✅/❌ block) is a completely separate channel and is never gated by this flag. `--log-level warn` or `--log-level off` silences them for one run; `BRICKKIT_LOG_LEVEL=warn` (or `off`) sets that as the default for a whole shell session without adding the flag everywhere. `debug` goes the other way, for troubleshooting |
