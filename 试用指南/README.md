...
> **只有两小时？** 走 [00a-两小时上手](00a-两小时上手.md) 那条主线（6 篇），
> 其余 16 篇按需回来看。下面是完整顺序。
...
| 11 | [11-组件签名.md](11-组件签名.md) | **cosign 签名发布、验签安装、篡改后被拦住、digest 钉住** | 09 |
...
| 17 | [17-网络策略与最小权限.md](17-网络策略与最小权限.md) | 依赖声明变成网络边界：入站、**出站**、拿掉 API 令牌 | 15 + 能执行策略的 CNI |
| 18 | [18-多项目与共享.md](18-多项目与共享.md) | **两个项目怎么共享**：先问状态在哪、共享资源、一个字段就隔离 | 05 |
| 19 | [19-多副本与优雅排空.md](19-多副本与优雅排空.md) | 一个组件跑几份，以及**运维换机器时不会把你的服务一次干掉** | 07 |
| 20 | [20-市场进阶.md](20-市场进阶.md) | 发出去之后：**谁能看见、谁能装、谁改过什么**（可见性 / 组织 / 访问策略 / 审计） | 09 |
...
| **要不要管出站**（会翻转默认行为） | `networkPolicy.egress` | [17.5](17-网络策略与最小权限.md) |
...
| **升级集群时服务会不会断** | 多副本 + 自动生成的 PDB | [19](19-多副本与优雅排空.md) |
| **组件只想给自己团队用** | private + 组织授权 | [20](20-市场进阶.md) |
| 出错了 | — | [10](10-排障速查.md) |
...
| 能力 | 状态 |
| --- | --- |
| HPA 自动扩容 | ⬜ 按 CPU/内存自动调整副本数（005 §5.8）。**手动 `replicas` 已支持**，多副本时自动生成 PDB，见 [19](19-多副本与优雅排空.md) |
