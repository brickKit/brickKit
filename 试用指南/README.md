...
| 走法 | 顺序 | 适合 |
| --- | --- | --- |
| **先跑通** | `01 → 02 → 05` → 回头看 `03`/`04` | 想尽快看到东西真的动起来 |
| **先理解** | `01 → 02 → 03 → 04 → 05` | 习惯先搞清模型再动手 |

`03`（依赖与启动顺序）和 `04`（组件开启模式）讲的是**平台怎么算出该启动谁**，
用的是 `brickkit order`——只算不启动，**不需要 Docker**。
它们不是 `05` 的前提；跑通 `05` 之后再回来看，你会有真容器可以对照。

> `04` 是整个平台**最容易想当然**的一篇。可以晚看，但别不看。

再往后按编号走：`06` 本地调试、`07` K8s（要 minikube）、`08` 升级、
`09` 市场（要把后端跑起来）、`11` 签名（接着 `09`，发布者侧还要装 cosign，使用者侧不需要）。
...
`17`–`20` 是"用起来之后"的事：网络策略、多项目共享、多副本、市场权限。
