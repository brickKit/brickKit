...
> **这不是一条流水线，随时可以推倒重来。** 某一节把配置试花了、或者你想换个顺序读，
> 执行 `./试用指南/准备.sh --reset` 再按
> [02 · 2.5 末尾](02-添加与移除组件.md#演示到此为止重置试验场)那四条命令，
> 就回到 03 往后统一使用的基准状态（五个组件）。有几节会主动让你这么做——
> 有些东西不重置就没法干净地演示。
...
| 05 | [05-Docker启动全流程.md](05-Docker启动全流程.md) | `up` / `status` / `down`、迁移、资源、建库 | 02 + Docker |
...
| 09 | [09-多副本与优雅排空.md](09-多副本与优雅排空.md) | 一个组件跑几份，以及**运维换机器时不会把你的服务一次干掉** | 00 + minikube |
...
| 13 | [13-完整装配.md](13-完整装配.md) | **八个组件一次装起来**：递归依赖、资源绑定、密钥、config 覆盖 | 00 + Docker |
