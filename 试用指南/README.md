...
| **K8s 升级后旧版本怎么办** | ⚠️ 目前要手工清（已知缺陷） | [08.5](08-升级与多版本.md) |
...
| 缺陷 | 表现 | 怎么绕 |
| --- | --- | --- |
| **K8s 升级留下旧版本**（P38） | 改版本号再 `up`，新旧两版同时在跑。`status` 只报新版本，`down` 也删不掉旧的 | [08.5](08-升级与多版本.md) 有手工清理命令 |
| **网络策略挡住监控**（P36） | 开了 `networkPolicy` 后 Prometheus 抓不到 `/metrics`，**指标悄悄停了**而服务正常 | [17.2](17-网络策略与最小权限.md) 有额外放行的策略 |
