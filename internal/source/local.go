...
func (s *localSource) listComponents() ([]string, []listProblem, error) {
	...
	for _, f := range files {
		...
		if err := yaml.Unmarshal(data, &h); err != nil {
			problems = append(problems, listProblem{f.id, "component.yaml 解析失败：" + err.Error()})
			continue
		}
		if h.Metadata.ID != f.id {
			problems = append(problems, listProblem{f.id,
				"目录名是 " + f.id + "，component.yaml 里却写着 " + h.Metadata.ID})
			continue
		}
		if !manifest.IsExactVersion(h.Metadata.Version) {
			...
			problems = append(problems, listProblem{f.id, "metadata.version 不是精确版本：" + got})
			continue
		}
		ids = append(ids, f.id)
	}
