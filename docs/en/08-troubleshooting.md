...
- **Cause:** no BrickKit schema is attached, so the YAML language server falls back to SchemaStore, a public catalog of schemas. As of yaml-language-server 1.24.0 and the catalog at the time of writing, it maps the file name `component.yaml` to Kubeflow Pipelines' schema (both are third-party and can change). Those messages are about Kubeflow's fields, not BrickKit's. (`brickkit.yaml` doesn't have this problem: as of September 2026, SchemaStore's catalog carries a `brickkit.yaml` / `brickkit.*.yaml` entry pointing straight at BrickKit's own schema, so an editor whose catalog cache has refreshed needs no setup for that file.)
