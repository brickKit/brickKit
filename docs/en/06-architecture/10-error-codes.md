...
| `Note: this component's shell isn't running this cycle, so it's deploying standalone from its own image instead of merging into the shell` | `CONFIG_INVALID` | The named shell is turned off (`mode: disable`, most often) this run. The member falls back to deploying on its own instead of merging in — its own image now has to actually be able to run standalone, and any resource dependency it declares needs its own binding to fall back on. If the component declares a migration, this run's warning also names that the migration now actually runs (it doesn't while merged) |
