...
| `brickkit.yaml 的 config 里可能写了明文密钥` | `CONFIG_INVALID` | A `config` key declared `secret: true` in the component's `configSchema`, or merely named like a secret, has a literal value (an `existingSecret` reference is not a literal and never triggers this). For a name-only match the check goes on the name alone, never the value; if it isn't a secret, ignore it |
| `existingSecret 写法不会生效：配置项没有声明 secret: true` | `CONFIG_INVALID` | A config value written as `{ existingSecret, key }` on a property that isn't declared `secret: true` — the shape is silently not injected, this names it |
