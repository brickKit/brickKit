...
| `configSchema.properties.<key>.secret` | boolean | no | default `false`. Unlike `enum`/`items`/`minimum`/`maximum`/`pattern` above, this one is **not** inert: it never validates the value, but it does decide **where the value is written**. Under `deploy.target: k8s`, a property with `secret: true` gets its own generated `Secret` (`secrets/config-secrets.yaml`, named `<versioned-service-name>-config-secret`) and the Deployment carries only a `secretKeyRef` — never the plaintext value. Under `docker`, this field currently has no separate effect (a compose `${VAR}` placeholder is already never expanded into the file). |
