...
This document owns the *fields*. What a field actually does once it's set correctly — how `enabled` cascades, how a resource binding turns into environment variables, how `servedBy` merges a member's config onto its shell — is covered by [environment-variables.md](environment-variables.md), [resource-binding.md](resource-binding.md), [dependency-resolution.md](dependency-resolution.md), and AGENTS.md §5; this document cross-references them rather than repeating them.
...
⚠️ **`publicKeys` is the field that actually makes signature verification take effect** — with zero keys configured, verification is disabled entirely regardless of `requireSignature`, and the CLI only warns about this once, not on every run. AGENTS.md §7 and [signing-and-trust.md](signing-and-trust.md) have the full reasoning for why a public key has to live in the project rather than travel alongside the signature from the marketplace.
...
- [environment-variables.md](environment-variables.md) — exactly what `config`, resource bindings, and `servedBy` turn into inside a container's environment
- [resource-binding.md](resource-binding.md) — binding-slot and same-kind-collision errors in full, plus the quota merge chain
- [dependency-resolution.md](dependency-resolution.md) — what the cascade and resolve stages actually do with `enabled`, required/optional dependencies, and diamond deduplication
- [component-yaml-reference.md](component-yaml-reference.md) — the component-side counterpart this file's `components[]`/`resources[]` entries bind against
- [signing-and-trust.md](signing-and-trust.md) — what `installer.publicKeys` actually gates and why
