...
🔒 marks a secret: on `deploy.target: k8s` this variable is generated as a `valueFrom.secretKeyRef` against a generated `Secret`, not a plain value (see [deployment-generation.md](deployment-generation.md)); on Docker it's a plain environment value like everything else, since Compose has no native secret object to delegate to.
...
This is the mechanism [resource-binding.md](resource-binding.md) uses to let one component bind two same-`kind` resources at once without a collision — full detail there; this document only owns the variable names themselves.
