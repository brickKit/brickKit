# 13. Network Policy and Least Privilege
...
`demo/caller` is `demo/hello`'s only dependent in this project, so it's the only pod selector in the generated ingress rule, on exactly the port `demo/hello` actually listens on — the platform already knows this from the same dependency graph [Dependency resolution and start order](../06-architecture/02-dependency-resolution.md) walks through in depth. `allowFrom` (AGENTS.md §7) exists for the case this doesn't cover: something outside this project's own dependency graph — an ingress controller, another team's namespace — that legitimately needs access despite not appearing as a declared dependency anywhere.
...
Next: [Multi-project sharing](14-multi-project-sharing.md), the last article in this series — sharing components across projects deliberately, using `brickkit fetch` from Article 8 for real, across two genuinely separate projects.
