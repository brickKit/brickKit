...
The migration ran as its own `Job`, completed, and only then did the two long-running Deployments come up — exactly the ordering [Deployment file generation](../06-architecture/03-deployment-generation.md) describes, now against a real API server instead of a generated file on disk.
