...
Every earlier article installed `demo/hello` from a `local` source — a directory on disk. This one runs a real marketplace (the same one [Self-hosting the BrickKit Market](../patterns/deployment/self-hosted-market.md) covers deploying), publishes `demo/hello` to it, and installs it into a completely separate project that never touches the component's source directory at all.
