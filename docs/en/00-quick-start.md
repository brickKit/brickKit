...
There's still one reason not to skip this for `component.yaml`: it's too common a file name for the shared catalog to reserve for BrickKit, so with no BrickKit schema attached, the YAML language server falls back to whatever else in SchemaStore, a public catalog of schemas, claims that name. As of yaml-language-server 1.24.0 and the SchemaStore catalog at the time of writing, that's Kubeflow Pipelines' schema — both are third-party and can change, so what your editor shows later may differ. In our check, a perfectly valid BrickKit `component.yaml` came out underlined almost everywhere (`Property apiVersion is not allowed.`, `Missing property "implementation".`). Attaching the BrickKit schema, either way below, replaces whatever it would have guessed.

`brickkit.yaml` doesn't have that problem: SchemaStore's catalog carries a `brickkit.yaml` / `brickkit.*.yaml` entry pointing straight at BrickKit's own schema (merged into the catalog in September 2026), so an editor that consults it needs no setup at all for that file, once its local copy of the catalog has refreshed. Attaching the schema by hand, below, still works and takes effect immediately regardless of catalog timing — and it's the only option for `component.yaml`.
