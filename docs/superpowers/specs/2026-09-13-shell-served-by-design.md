...
type Member struct {
    ...
    Env  []inject.Var      // 该组件"如果独立部署会拿到"的完整环境变量
}
...
**重构 `internal/inject`**：把 `Build` 主循环内"单个组件的环境变量怎么算"这段逻辑拆成独立可复用函数（如 `BuildFor(ref, ...) []Var`），`Build` 本身改为对每个运行中组件调用它。`internal/shell.Resolve` 对每个 member 调用同一个函数，取得它"如果独立部署"本该拿到的完整环境变量集合，再合并进外壳。

**合并规则**：以 name→value 增量构建一个 map，先放外壳自己的变量，再逐个 member 合并——同名同值（例如两个成员恰好依赖同一个第三方组件）跳过，不算冲突；同名不同值，报错并指出是哪两个组件、哪个变量名冲突。

`labels`（`brickkit.yaml` 组件级）用同样的合并规则处理：同名同值跳过，同名不同值报错，与 env 的处理方式保持内部一致，不特殊化处理成"直接不支持"。
...
- 外壳组件的 compose service 照常生成，额外做两件事：
  ...
  - 把 `internal/shell.Resolve` 算出的合并环境变量写进这一个 service 的 `environment:`。
...
- member 若声明了 `healthCheck`，发一条警告：这个字段不会生效，健康检查由外壳负责聚合。
...
be-assembly-standard 反馈的 Go/protobuf 全局注册表撞车问题，本质是"进程级全局唯一注册表 + 同一份生成代码被独立复制多份"的通用性质，不是 brickKit 需要处理的问题——如果按第 6 节的设计，平台直接把所有被收编组件的环境变量注入外壳容器的 OS 环境（而非指望外壳内部再手动 relay 一遍），第 6 条提到的"环境变量没有被 relay 进外壳内部进程"这类坑对遵循这个约定的外壳实现者不会存在，值得写进指南作为佐证。
...
- `internal/shell`：表驱动单元测试覆盖全部校验规则（目标不存在、自指、链式嵌套、端口冲突、env 合并同值跳过/异值报错）。
- `internal/inject`：`BuildFor` 拆分后的行为与原 `Build` 循环逐组件产出一致（回归）。
