...
6. **不做语言级技能**（`references/go.md`、`python.md` 之类的语言适配层）。
   与第 1 条同源：能写的只是一层极薄的适配说明（这门语言里怎么读注入的环境变量、
   Dockerfile 暴露哪个端口），而它一旦落成散文就开始腐烂，且 CI 跑不到。
   真要做语言相关的东西，正确形态是让 CLI 生成一个**被 CI 真构建过**的组件骨架，
   技能里只留一句「用那个命令生成，照着改」——一句话不会过期。
   那属于另一个特性，不在本设计范围内，也不作为本设计的后续计划。
7. **不写使用者的 `CLAUDE.md`**（哪怕只有一行导入）。那是他自己的流程文件，
   而 skill 的 `description` 常驻、不靠它也能被发现——侵入换来的收益太小。
   见 §4。
8. **技能正文不规定流程**。只讲机制与禁区。使用者有自己的做法，我们只保证他
   不踩坑。见 §7。
...
1. **Claude Code 不读 `AGENTS.md`。** 文档原文：「Claude Code reads `CLAUDE.md`,
   ...
   但结论**不是**「那就替他写一份 `CLAUDE.md`」——见 §4 与 §11 第 7 条：
