...
1. 不改变 `sync` 的任何行为，也不给它加参数（§3.9「没有参数」的口径不动）——
   **但 2026-09-06 gap report 之后有一处例外**：`sync` 检测到目标是已登记
   （`.gitmodules` 里有记录）的 git submodule 时会阻断而不搬，见下条。
2. 不做 `--dry-run`、不做卸载命令（删掉那个 hook 文件就是卸载）。
3. 不管嵌套 Git 仓库（gitlink）该不该存在——只在检查时附带一句提醒（§4.6）。
   **这条只覆盖"没有 `.gitmodules` 登记"的意外死 gitlink**；已登记的真
   submodule 是第三种、当时没设计过的形态（2026-09-06 gap report），现在
   `restore --check` 认得出它、不再误报，`sync`/`remove` 遇到它会直接阻断
   并给出手工步骤，而不是"只提醒不处理"。
