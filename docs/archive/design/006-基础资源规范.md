...
这张表只覆盖**连到哪、用什么身份**——那部分任何同类产品都有，平台才敢替你注入。产品特有的旋钮由组件自己声明：

| 谁管 | 什么 | 怎么声明 |
| --- | --- | --- |
| **平台** | host / port / username / password + 占哪一块（库名、vhost、bucket、index） | `resources` + `bindings` |
| **组件** | 产品特有的：Kafka 的 `acks` / 分区数、RabbitMQ 的 `prefetch`、S3 的 `region` / path-style、SMTP 的 TLS 模式 | `configSchema`（002 §6.5） |

**这条界线不是"平台偷懒"，它换来的东西 `configSchema` 给不了：**

| `resources` 给的 | 只用 `configSchema` 时 |
| --- | --- |
| 密码走 K8s Secret，不明文进 Deployment（005 §5.6） | ❌ 明文 |
| 一处声明、多组件共享——一个 MQ 给十个组件用，改密码改一处 | ❌ 连接信息抄十遍 |
| `up` 列出"这些得先跑起来"（§9.1） | ❌ |
| K8s egress 策略从 `resources[].port` 推导（005 §5.13） | ❌ 得手写 |

所以将来真要接 Kafka、MinIO、Elasticsearch 时，**照常用 `kind`**，产品特有的那几项走 `configSchema` 补上。

⚠️ **可选槽位带着某个产品的口音，不填就是了。** `MQ_VHOST` 是 RabbitMQ 的概念——Kafka 与 NATS 没有 vhost，绑定里不写 `vhost` 即可（空值不注入，组件读不到这个变量）。`SEARCH_INDEX` 同理：很多 ES 客户端按查询指定 index，不需要连接级的。
...
**kind 是封闭枚举，engine 是自由字符串。** 但"自由"指的是**取值不受限**，
不是"平台不看它"——两者的分工是：

| | 决定注入哪组变量 | 参与匹配 |
| --- | --- | --- |
| `kind` | ✅ `database` → `DATABASE_*`、`cache` → `REDIS_*` | ✅ |
| `engine` | ❌ 平台不认识 rabbitmq 与 kafka 有什么不同 | ✅ **逐字相等** |

⚠️ **`engine` 参与匹配，而且是逐字相等。** 组件在 `dependencies.resources` 里写
`engine: postgresql`，项目在 `resources` 里写 `engine: postgres`，平台就认为
**这不是同一样东西**，`up` 会阻断（§4.4）。它不认别名，也不做归一化——
`postgres` 与 `postgresql` 在它眼里是两个不同的值。

这道闸门是有意留的：项目里同时有 postgres 与 mysql 时，它是平台唯一能看出
"组件要的和管理员绑的不是同一样东西"的依据；而绑定是管理员显式写下的，
这个不一致平台看得见，就不该放过去。

代价是那个词要在**两个人写的两份文件**里逐字相同，所以报错必须把两个词都摆出来：

```
demo/hello@1.0.0：需要 kind: database、engine: postgresql（资源 pg-main 已经绑给它了，但它的 engine 写的是 postgres）
```

> 这一段从前写的是"平台对 engine 一无所知——它只是写给人看的"。那句话与本节
> 下面的 §4.4（"kind + engine 必须与组件声明的一致"）以及实现直接矛盾，而实现
> 按的是 §4.4。撞上的人会看到一句**假话**——"brickkit.yaml 的 resources 中未声明"
> ——然后去翻 `sources` 找那个"没声明"的东西，可他明明声明了、也绑了，
> 只是把 postgresql 写成了 postgres。
...
> **这里没有 `database`。** "这个组件占哪一块"写在 `bindings[]` 上，不在资源上——
> 一个资源给多个组件用时，每个组件占的库/vhost/bucket 各不相同（§5.2）。
> 这一行从前在表里，而 CLI 会当场拒绝它：`resources[0].database：未知字段`。
...
❌ 错误：资源依赖未满足
   ...
   demo/hello@1.0.0：需要 kind: database、engine: postgresql（资源 pg-main 已经绑给它了，但它的 engine 写的是 postgres）
   建议：
   1. 没声明这一类资源 → 在 brickkit.yaml → resources 中加一条
   2. engine 写的不一样 → 改两处中的一处让它们逐字相同（平台不认别名：postgres 与 postgresql 是两个不同的值，006 §4.4）
   3. 声明了但没绑 → 在该资源的 bindings 中加一行 componentId
   4. 暂时不想跑这个组件的话，给它写 enabled: false
```

**三种不满足指向配置里三个不同的地方**，所以每条明细都写明是哪一种：

| 明细说的 | 该改哪儿 |
| --- | --- |
| `resources 中未声明` | 这一类资源压根没有，加一条 |
| `engine 写的是 X` | 声明了、可能也绑了，只是那个词不一样 |
| `已声明，但未绑定给该组件` | 资源对，缺一行 `componentId` |

**只查本次会启动的组件。** 被 `enabled: false` 关掉、或跟着上层一起不启动的组件不参与——
...
| 变量 | 来自 |
| --- | --- |
| `*_HOST` / `*_PORT` | 资源声明的 `host` / `port` |
| `STORAGE_ENDPOINT` | `host:port` 拼起来的完整地址 |
| `*_USER` / `STORAGE_ACCESS_KEY` | 资源声明的 `username` |
| `*_PASSWORD` / `STORAGE_SECRET_KEY` | 资源声明的 `password`（K8s 下走 Secret，不明文进 Deployment） |
| `DATABASE_NAME` / `MQ_VHOST` / `STORAGE_BUCKET` / `SEARCH_INDEX` | **绑定**里那一格：`database` / `vhost` / `bucket` / `index` |

最后一行是**每条绑定各自的**——同一个 PostgreSQL 绑给三个组件，三个库名各写各的（§7）。前面几行来自资源本身，所有绑定它的组件拿到的都一样。

#### 「占哪一块」四个名字，按 kind 用对应的那个

```yaml
bindings:
  - {componentId: people/basic, database: people}     # kind: database
  - {componentId: shop/order,   vhost: orders}        # kind: mq
  - {componentId: media/store,  bucket: media-prod}   # kind: storage
  - {componentId: search/api,   index: products}      # kind: search
```

它们在内部是**同一格**，分成四个名字是因为使用者第一反应写的就是他心里那个词。用错了会被点名：

```
❌ resources[1].bindings[0].database：kind: mq 下这一格叫 vhost，不是 database（注入为 MQ_VHOST）
```

`cache` 与 `smtp` **没有这一格**（Redis 与 SMTP 的连接变量里不存在对应的项）。写了同样报错——写了不生效比报错难查得多（003 §3.2）。

⚠️ **`STORAGE_ENDPOINT` 带端口。** 从前它只取 `host`，而 `port` 是必填并被校验的——使用者被要求填 `9000`、通过校验，然后那个端口原地蒸发，组件拿着 `minio.internal` 去连 MinIO 连不上，而配置看上去完全正确。
...
因此 CLI 执行 `up` 时**必须显式传 `--project-directory <项目根>`**——那一步要照着部署文件创建容器，compose 得插值 `${VAR}`，而它默认只在**部署文件旁边**找 `.env`。

`down` / `ps` / `logs` 不需要它：那几条不带 `-f`，compose 从容器标签就认得出项目，没有文件要插值，也就没有 `.env` 要找。
...
⚠️ **这张表的排序只在 PostgreSQL / Oracle 上才成立。** "database"和"schema"
这两个词在不同引擎里指的不是同一档东西——在 MySQL 上它们干脆是同义词。
照搬这张表会在某些引擎上买到比你以为的弱一档的隔离，见 §7.7。

至于"换成 schema 分组能不能省连接数"——**默认部署形态下不能**，理由见 §7.8。

§9.5 那句「一个组件一个数据库」说的就是这张表的第一行，是**默认建议**而不是禁令：
多条 `bindings` 写同一个 `database` 值是支持的写法，平台按库名归集去重
（每次 `up` 列出的待建库清单里它只出现一次）。
...
### 7.7 database 与 schema：同一个词，在不同引擎里不是一回事

§7.2 那张表说"不同 database ⭐推荐、不同 schema ✅可用"，读起来像是一条
跨引擎通用的强弱排序。**它不是。** 两个词在不同引擎里指的根本不是同一档东西。

判断标准只有一条：**一条连接能不能跨过去。**

| 引擎 | 硬边界（连接跨不过去） | 软边界（连接能跨，只靠权限拦） | 跨库要什么 |
| --- | --- | --- | --- |
| **PostgreSQL** | `database` | **`schema`** | dblink / postgres_fdw |
| **Oracle** | `PDB`（12c+） | **`schema`（== user）** | DB Link |
| **MySQL / MariaDB** | 无（实例才是） | **`database`（`CREATE SCHEMA` 是它的同义词）** | 不需要，`db.tbl` 直接跨 |
| **SQL Server** | 无（实例才是） | `database` + `schema` 两层 | 不需要，`db.schema.tbl` 直接跨 |
| **MongoDB** | 无 | `database` / `collection` | 不需要，一个 client 通吃 |
| **ClickHouse** | 无 | `database` | 不需要 |
| **Redis** | 无 | 编号 db 0–15（**cluster 模式只有 0**），实践中用 key 前缀 | `SELECT n` 切换 |

**所以对应关系是这样的：**

```
PostgreSQL 的 schema   ≈   MySQL 的 database   ≈   Oracle 的 schema
PostgreSQL 的 database ≈   Oracle 的 PDB       ≈   MySQL 的另一个实例
```

这带来一个文档不说清楚就会踩的错位：**同一句"一个组件一个 database"，
在 MySQL 上买到的隔离强度等于 PG 的 schema，而不是 PG 的 database。**
MySQL 那边一条连接凭权限就能 `people.employees JOIN department.nodes`，
PG 那边连不上——差一整档。

平台对此不做任何事（它只注入 `DATABASE_NAME`，见 §5.2），但**使用者要知道
自己买到的是哪一档**：靠"数据库分开了"就以为组件之间物理上读不到彼此，
这个假设在 MySQL / SQL Server / Mongo 上不成立，那里挡住越界的从来只有权限。
§7.5 的"每个组件一个独立账号"在这些引擎上不是加分项，**是唯一的那道墙**。

> **顺带一条实践差异：** PG 上一个组件一个 database 时，扩展是装在库里的——
> 30 个库要装 30 遍 `uuid-ossp` / `pgcrypto`；autovacuum 也是按库轮的，
> `autovacuum_naptime`（默认 60s）要摊到所有库上，库越多每个库被光顾的间隔越长。
> 这些是"多库"的运维面成本，与隔离强度无关，但在库的数量上百时会真的开始疼。

### 7.8 schema 分组**不会**减少连接数

这是一个看起来天经地义、但在 BrickKit 的默认形态下**不成立**的推论：
"把 30 个 database 换成 1 个 database 里的 30 个 schema，连接数就少了。"

**连接数由「进程数 × 池大小」决定，不由「库的个数」决定。**

平台的默认形态是一个组件一个容器一个进程（005 §2.5）。30 个组件就是
30 个进程、30 个**各自独立**的连接池。把它们指向同一个库的 30 个 schema，
这 30 个池仍然是 30 个池——它们在不同进程里，**没有任何机制让它们合流**。

想省连接，只有两条路，各自都有账要付：

| 路 | 能省吗 | 代价 |
| --- | --- | --- |
| **多组件跑在同一个进程里共用一个池** | ✅ 能，而且省得多 | 就是合并部署。**平台不提供任何支持**（012 §2.21），全部自理。这条路上 schema 分组是**必要条件**——见《组件合并部署》第 7 章 |
| **前面挂 PgBouncer 之类的池化器** | ⚠️ 有条件 | **PgBouncer 的服务端池按 `(user, database)` 分。** schema 分组只合并了 `database` 那一维，`user` 那一维没动——而 §7.5 / §11.1 / 008 §15 要求每个组件一个独立账号。**30 个账号 × 1 个库 = 还是 30 个服务端池，一个都没省** |

第二行是要害：**要靠池化器真省下连接，代价是放弃"每组件独立账号"。**
这是最小权限与连接复用之间的一个真实取舍，不是免费午餐——而在
§7.7 表里那些"没有硬边界"的引擎上，独立账号是唯一的那道墙，放弃它
等于把数据边界一起放弃了。

**那 schema 分组省的是什么？** 省的不是连接，是服务端开销与运维面：
每库一份系统目录与统计信息、backend 的 relcache 不能跨库共享、扩展要装 N 遍、
autovacuum 按库轮、备份与配额按库管（§7.7 末尾那条注）。这些是真实收益，
只是**和连接数无关**，别把两笔账记成一笔。
...
早先有个 `brickkit up --check-resources`（已删除），做的正是"对 `host:port` 拨一次 TCP"。
删它的理由，与下面这张"为什么不做 SELECT 1"的表
其实是同一条，只是推到了底：
...
**默认建议：一个组件一个数据库。** 共用同一个 database 意味着一个组件能读到
另一个组件的表，违反 002 §2.2 的数据自治。

⚠️ **这是一条默认建议，不是约束。** 单库 + 每组件独立 schema + 独立 Role
（PG 上的第二方案，§7.2 表里的「✅ 可用」）达到的隔离强度是被认可的，
`bindings` 里多条写同一个 `database` 值也是明确支持的写法——平台按库名归集去重。

之所以默认推荐一库一组件，不是因为 schema 不行，而是因为**它不需要理解三件事**：
迁移工具默认往 `public` 里写、`search_path` 设完不能就把连接还回池子、跨 schema
的越界只有权限拦得住。三样漏一样，症状都是「不报错，只是悄悄读写了别人的数据」。
一个库一个组件不需要懂这些——连接绑在库上，越界在 PG 上物理不可能。

选 schema 分组之前先看两处：**跨引擎的档位对照**（MySQL 上 database 与 schema
干脆是同义词，照搬 §7.2 那张表会买到弱一档的隔离）在 §7.7；**「schema 分组能省
连接数」这条推论为什么在默认部署形态下是错的**在 §7.8。完整决策见 012 §2.22。

它还有**时序性**：库按 database 建好、数据进去之后再改成一库多 schema，就是一次
数据迁移。预见到将来可能走合并部署的项目（JVM 为主 + 私有化交付），一开始就选
schema 才是便宜的——那时它是把 N 个连接池并成 1 个的**必要条件**
（《组件合并部署》§7）。平台不推荐它，但必须让人在还来得及的时候知道有这个岔路口。
