...
components:
  - id: people/basic          # 组件 ID（必须）
    ...
    labels: {}                # 部署元数据透传（可选，平台不解释键值，见 §4.11）
```
...
`*_ENDPOINT`，按 002 §3.4 自行降级。`up` 会点名说清是谁失去了什么——
**是信息不是警告**，因为这正是本节推荐的做法（004 §4.5）：
...
⚠️ **`local: true` 的组件上 `expose` / `exposePort` 不生效。** 它不生成容器，
平台没有端口可以映射到宿主机——那个进程已经在宿主机上了，地址是
`localhost:<localPort>`（不写 localPort 时取组件声明的 `deployment.port`）。
`up` 会警告一句并直接给出真实地址：
...
**为什么是警告，而 `local` + `replicas` 是报错**（§4.10）：后者真的互相矛盾——
IDE 里只有一个进程，没有任何说得通的读法。而 `expose` 与 `local` 不矛盾：
那个进程确实对外可达，只是这件事不再由平台来做，错的只有"在哪个端口"。
报错会打断一个很正常的流程——给一个长期 `expose` 的前端组件临时加上
`local: true` 去调试。
...
**两个环境各有一样"只能归一个组件"的东西，抢了都在生成阶段报错。**

| 环境 | 独占的是什么 | 抢了怎么办 |
| --- | --- | --- |
| Docker | **宿主机端口** | 给其中一个设 `exposePort` |
| K8s | **域名**（`hostname`） | 给其中一个换个子域名 |
...
**K8s 环境不需要 `exposePort`：** 两个组件可以共用同一个容器端口（如 80），
只要 `hostname` 不同（`portal.example.com` vs `admin.example.com`）就不冲突。
换句话说，K8s 下抢的不是端口，是域名——完整说明见 005 §5.5.0。

⚠️ **多版本共存时最容易撞域名：** 照 003 §8.3 加第二个版本时整个条目是复制出来的，
`hostname` 会跟着一起复制。
...
| 字段 | 类型 | 必须 | 说明 |
| --- | --- | --- | --- |
| labels | object | ❌ | 透传给底层引擎的部署元数据。平台**不解释键值**。Docker → service 的 `labels`；K8s → Deployment 与 Pod 的 `annotations`。值必须是字符串 |

**这个字段存在的理由：平台不做网关，所以必须让你自己做。**

Traefik / Caddy / Prometheus / Loki 这一整类工具的标准接入方式就是读容器标签。
平台不做 API 网关、不做路由表（012 §2.1、005 §5.11）——这条立场本身没问题，
但如果**同时**不给透传口，「平台不做路由」就变成了「也不让你自己做」：
你只能退回去手写一份 file-provider 配置，而那份配置里必须写满**版本化服务名**
（`erp-sales-1-0-0`），于是组件每升一次版本都要同步改一遍，
**而平台一个字都不会提醒**。组件一多，那份手写配置是必然会先腐烂的一份。

有了这个口子，「不做网关」这条立场才真正成立而不是变成一堵墙。完整论证见 012 §2.23。

**优先级：** `brickkit.yaml components[].labels` **逐键覆盖** `component.yaml deployment.labels`（002 §4.7）。

逐键而不是整块——与资源配额同一个理由（§4.7）：只想加一条路由规则，
不该因此把组件作者写的 `prometheus.io/path` 一起丢掉。

```yaml
# component.yaml（组件作者）        # brickkit.yaml（项目）        # 生效
# prometheus.io/scrape: "true"     # prometheus.io/scrape: "false" → "false"
# prometheus.io/port:   "9090"     #                               → "9090"
#                                  # traefik.enable:       "true"  → "true"
```

**平台拥有的键，写了就报错**（`app`、`brickkit.io/*`、`com.docker.compose.*`）——
理由与完整表格见 002 §4.7。撞了当场报错而不是静默丢弃：静默丢弃的症状是
「labels 写了但没生效」，那时人会去翻 Traefik 的日志，而那里什么都没有。

⚠️ **值必须加引号。** `traefik.enable: true` 会被 YAML 读成布尔，当场报错并告诉你补引号。

⚠️ **`local: true` 的组件上写了 labels 会收到警告。** 那个组件不生成容器
（跑在 IDE 里，§4.4），没有可以挂标签的对象。合并部署交付时这很常见
（《组件合并部署》§4.3）——真正要挂标签的是你自己写的那个外壳，
写在你那份 compose 文件里。
...
#### 6.4.1 一个组件目录只放得下一个版本

目录结构是 `<path>/<scope>/<name>/component.yaml`——**路径里没有版本这一层**。
所以本地安装源在任何时刻只能给出**一个**版本：那份 `component.yaml` 里写的是几，
它能给的就是几。这不是限制，是"它就是你正在编辑的那份源码"的直接后果
（一个组件一个仓库，002 §1.6；`--repo` clone 下来的就是这个目录）。

**本地开发升级组件时，这一点会以一种不明显的方式冒出来。** 把
`components/department/tree/component.yaml` 从 1.0.0 改成 2.0.0 之后：

| 谁 | 从哪来 |
| --- | --- |
| `department/tree@2.0.0` | 本地源那份文件 |
| `department/tree@1.0.0`（还有别的组件依赖着它） | **只剩 `.brickkit/manifests/` 缓存里那一份** |

只要缓存还在，一切照常——两个版本自动共存，老调用方继续连 `department-tree-1-0-0`，
你可以从容地一个个适配（002 §7.8：升级被依赖组件后调用方不会自动切换）。

`.brickkit/manifests/` 默认是**提交进 Git** 的（§11 的 `.gitignore` 把它注释掉了），
所以正常协作不会踩到。会踩到的是把那两行注释放开、`rm -rf .brickkit`、
或者在一个全新目录里重来——那时旧版本谁也给不出来了：
...
❌ 错误：强依赖缺失
   ...
   安装源 local-dev（local）：这里是 2.0.0
   建议：
   1. 本地安装源一个组件目录只放得下一个版本（003 §6.4）——要的那个版本只可能在 .brickkit/manifests/ 缓存里，而缓存是可以被删掉的
   2. 要让依赖 department/tree@1.0.0 的组件继续跑，把它的依赖改到源里那个版本并适配
   3. 或者把安装源里那份 component.yaml 改回 1.0.0（新版本改从别处装）
```

> 这条报错从前是一句"该组件在所有安装源中均未找到"，配三条通用建议
> （查安装源配置 / 查有没有发布到市场 / 查版本号）——没有一条说到点子上，
> 而使用者会去翻 `sources`，问题却在他自己刚改过的那份 `component.yaml` 里。
> CLI 其实读到了那个文件、也看见里面写着 2.0.0，只是把这个信息丢掉了。

**有市场时没有这个问题**：1.0.0 一直取得到，缓存冷热都无所谓。
...
例外有两个：`credentials`（重新 `brickkit login` 即可），以及 `skills.lock`——
`brickkit skills update` 会把没被改过的那些文件重新登记回来，但**改过的那些会从此
按「未托管」处理**（CLI 再也无法确认它们原本是不是自己写的），所以它值得跟着项目提交。
