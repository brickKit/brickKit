...
**"另一个文件"怎么跟着组件走：声明成 `artifacts`。** 上面那句话只说了一半——
光放进仓库，闭源组件（只有 Manifest + 镜像引用）就带不走它。`artifacts` 正好补上
这一半：它随 Manifest 一起登记、闭源组件由市场存储（007 §11.4）、`add` 与 `fetch`
都会把它下载到 `.brickkit/artifacts/<版本化服务名>/<type>/`（004 §3、§4），
**而 CLI 只下载、不解析**——正是这类元数据要的语义。

约定用 `type: metadata` 放这类"给自己的工具链看"的文件（`type` 是自由字符串，
平台不校验，见 §2.3）。取一个约定名不是平台的规则，是让生态里的人用同一种做法，
而不是各发明一套：
...
artifacts:
  - type: metadata
    format: yaml
    description: "我们自己的代码生成器要读的一份声明"
    files:
      - .mytool/component-extra.yaml
```

**两个口子的分工**：要给**底层引擎**带一段元数据（网关路由、监控抓取），
用 `deployment.labels`（§4.7）——那是键值、平台会搬进生成物；要给**自己的工具链**
带一份**文件**，用 `artifacts`——那是文件、平台只搬运不打开。
两者都不要求平台理解多出来的东西，这正是它们与"扩展字段"的区别。
...
| 也是自定义元数据的分发载体 | 正因为"只搬运不打开"，`artifacts` 是把**自己工具链要读的文件**跟着组件带走的正路（§2.2.1）。约定 `type: metadata`。闭源组件靠它才带得走这类文件——那时组件只有 Manifest + 镜像引用，仓库里放的东西一样都跟不过来 |
...
| labels | object | ❌ | 部署元数据，平台**不解释键值、只透传**（§4.7）。Docker → service 的 `labels`；K8s → Deployment 与 Pod 的 `annotations`。值必须是字符串 |
...
### 4.7 部署元数据透传（deployment.labels）

组件作者可以声明一组**平台不解释、只透传给底层引擎**的部署元数据：
...
| 目标 | 写进哪里 |
| --- | --- |
| Docker | 组件 service 的 `labels` |
| K8s | Deployment 与 **Pod 模板**的 `annotations`（不是 labels，见下） |

**规则：**

| 规则 | 说明 |
| --- | --- |
| 平台不解释键值 | 与 `deployment.resources`「CLI 透传，不校验数值合理性」是同一种姿态。键名认不出、值写得没意义，都由下游工具自己报 |
| brickkit.yaml 可覆盖 | 项目在 `components[].labels` 里**逐键**覆盖（003 §4.9）。只改一个键不会把其余的一起丢掉 |
| 值必须是字符串 | Docker 的 labels 与 K8s 的 annotations 两边都只接受字符串。`traefik.enable: true` 少了引号会**当场报错**并告诉你补上（见下） |
| 不进迁移容器 | 迁移不是服务。同一份 labels 写上去，Traefik 会把那个跑完就退出的一次性容器当成路由目标 |
| 平台自己的键拒绝覆盖 | 见下表。撞了当场报错，不静默丢弃 |

**平台拥有的键，写了就报错：**

| 键 | 为什么归平台 |
| --- | --- |
| `app` | K8s 下 Deployment 找到自己 Pod 的唯一依据，也是 NetworkPolicy 的匹配依据（005 §5.3、§5.13.1） |
| `brickkit.io/*` | 平台自己的命名空间：组件 ID、版本、项目名、角色都记在这里 |
| `com.docker.compose.*` | docker compose 自己写的标签。覆盖后 compose 认不出自己生成的容器 |

⚠️ **少了引号是这个字段最常见的错。** 照着 Traefik 的文档敲最容易丢的就是那对引号：

```
❌ 错误：component.yaml 校验失败
   deployment.labels.traefik.enable：值必须是字符串，加上引号写成 "true"（第 12 行）
```

没有"帮你转成字符串"这个选项——转了就得决定 `1.0` 该变成 `"1"` 还是 `"1.0"`，
而那是平台开始解释键值，正是这个字段声明不做的事。

#### 为什么 K8s 下是 annotations 而不是 labels

两条理由，各自都足够：

| | |
| --- | --- |
| **会打架** | 平台的 `app: <版本化服务名>` 是 Deployment 选择器与 NetworkPolicy 的匹配依据。透传键进了 labels，一旦撞上就是**选择器选空**或**策略放行错对象**——两者都不报错 |
| **写不进去** | K8s 的 label **值**只许 `[A-Za-z0-9._-]`，而要透传的值里全是斜杠、反引号与括号（`` PathPrefix(`/erp/sales`) ``）。写进 labels 会被 API Server **整份拒绝**，连 Deployment 都建不出来 |

而 K8s 生态里真正需要透传的那些（`prometheus.io/*`、各类 CRD 提示）本来就是 annotation。
Pod 模板上也要写一份：`prometheus.io/scrape` 这一类抓的是 Pod，只写在 Deployment 上等于没写。

**这个口子是为了让平台可以继续不理解网关。** 完整论证见 012 §2.23。
...
| 升级兼容性检查          | **没有专门的一套**：换版本号只是让新 Manifest 走日常解析——依赖可满足、资源已绑定、无环，与首次 add 同一条路。不检查 API 兼容性（见 7.7）    |
