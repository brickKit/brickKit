...
| 保留期限 | 建议至少保留 1 年 |
| 可查询 | `GET /api/v1/audit`（需要认证）。市场没有 Web 界面，见 007 §3.6 |
...
deployment:
  resources:                       # 推荐配额，CLI 透传不校验（002 §4.6）
    requests: { cpu: "100m", memory: "128Mi" }   # 建议只声明 requests
```
...
市场管理员可以将某个版本标记为 blocked——`PUT /api/v1/components/{id}/versions/{ver}`（007 §9.2）：
...
`componentId` 与 `version` 在路径里，不在请求体里。`reason` 会记进审计日志的 `detail`（007 §16.2）：**一定要带上**。这是平台唯一的强制手段（§1.4 信任模型：安装即信任，平台只做最后的 blocked 下架），而"当初为什么下架"只有这一处能留下答案。

CLI 没有对应的子命令，只能直接调 API。
...
| 资源配额 | 只有 `requests` 有默认值（100m/128Mi），`limits` 不设 | Manifest 可声明推荐值，brickkit.yaml 可覆盖（005 §2.5） |
