...
上面那条 `brickkit publish` 就是全部。市场是一个纯 HTTP API 服务，**没有 Web 界面**——这一节从前描述的那套"登录 → 填表 → 上传 → 点发布"的流程从来没有存在过。理由与将来可能怎么补，见 007 §3.6。

不用 CLI 时可以直接调 API（007 §9 有完整清单），但那是给脚本用的，不是给人用的。
...
如果组件在 brickkit.yaml 中恢复启用（启停判定后会启动），执行 `brickkit sync` 会自动将其从归档目录移回活跃目录：
...
| 发布方式           | 只有 CLI `brickkit publish`。市场没有 Web 界面（007 §3.6）                  |
