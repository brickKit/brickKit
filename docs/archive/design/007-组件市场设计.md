...
**但"没有前端"与"不需要前端"是两回事。** 发布这件事确实不需要：它发生在开发者刚构建完镜像的那个终端里，`brickkit publish` 已经在那儿了，做成表单只是把一件顺手的事搬到另一个上下文里。

**真正需要界面的是发现**（"有什么可以装"）——要翻、要筛、要看描述和版本列表、要在几个候选之间比较。这些在终端里做出来都是二等公民。所以市场前端会做，而它的第一职责是发现，不是发布（§17.2 记了这个分工，以及为什么**刻意不做** `brickkit search`）。
...
**"有什么可以装"这一问归市场前端。** 001 §6 给市场的定位是回答两个问题——
"有什么可以装？谁有权装？"。第一问是**发现**，而发现天生适合用界面做：
要翻、要筛、要看描述和版本列表、要在几个候选之间比较。这些在终端里做出来
都是二等公民，而做好它等于在 CLI 里重新发明一个浏览器。

第二问（"谁有权装"）与安装本身则是**终端里的事**，`add` / `fetch` / `publish`
已经在那儿了。这条界线是清楚的：**装归 CLI，逛归前端。**

> **这一节从前写的是"这是一个已知缺口"，并说"补的形状应当是一个 `search`
> 子命令"。** 那个判断的前提是 §3.6 当时的结论——市场不会有前端，所以发现
> 只能落到 CLI 上。前端既然要做，这个前提就不成立了：
> 再加一条 `brickkit search` 等于让同一个问题有两个入口，而其中一个永远比另一个弱，所以**刻意不做**。
>
> 更早的版本还写过"通过市场 Web 界面或直接调用市场 API 完成"——那时界面并不
> 存在（§3.6），是在用一个不存在的东西论证另一个东西不必做。**现在不一样：
> 它是一个明确要做的东西，而不是一句托词。**
      ...
      "conflicts": [
        {
          ...
          "suggestion": "请修改配置项名称，避免与平台保留变量冲突（如改为 departmentTreeBaseUrl）"
        }
      ]
    }
  }
...
- 组件搜索走市场 API（`GET /api/v1/components`）与市场前端。**CLI 刻意不做 `search`**：装归 CLI，逛归前端（§17.2）。
...
| 组件搜索         | 走市场 API（`GET /api/v1/components`）与市场前端。CLI 刻意不做 `search`——装归 CLI，逛归前端（§17.2） |
