...
| 6 | 管理组件源码工作区 | `brickkit add --repo` / `--repo-all` clone 组件源码；`brickkit sync` 把这次不启动的组件源码收进 `components/.archived/`（判据与 `brickkit up` 一致）；`brickkit remove` 自动删除源码目录（含已归档的那一份） |
...
| `brickkit add <组件>[@版本]` | 添加组件，递归拉取依赖，写入 brickkit.yaml。不写版本时装安装源上最新的可安装版本；`--local` 把本地安装源里的组件一次全部添加；`--repo` / `--repo-all` 额外 clone 源码 |
| `brickkit fetch <组件[@版本]>` | 只下载组件的产物（API 契约等），不写入配置、不部署（跨项目调用，003 §4.9） |
| `brickkit remove <组件>` | 移除组件（有依赖方时阻止），自动删除源码目录（含已归档的那一份） |
| `brickkit up` | 生成部署文件并一键启动 |
| `brickkit down` | 一键停止（不删除 volume，数据安全） |
| `brickkit status` | 查看组件运行状态（读取底层引擎） |
| `brickkit sync` | 整理组件源码工作区：把这次不启动的组件源码收进 `components/.archived/`，该回来的移回来。判据与 `brickkit up` 完全一致 |
...
- ✅ 迁移失败时阻断主服务启动
- ✅ `brickkit status` / `down` 行为正确（`reset` 连同整套备份机制已删除，见 003 §7.2）
...
- ✅ `brickkit sync` 按启停判定结果双向归档/激活组件源码
