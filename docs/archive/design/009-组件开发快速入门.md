...
# 查看日志
docker compose -p brickkit-my-first-project logs hello-world-1-0-0
```
...
如果你的 configSchema 中声明了名为 `departmentTreeEndpoint` 的配置项，转大写后为 `DEPARTMENT_TREE_ENDPOINT`，与保留模式 `*_ENDPOINT` 冲突。市场在发布时会拒绝该组件（见 007 组件市场设计第 18 节）。请修改配置项名称，如改为 `departmentTreeBaseUrl`。

⚠️ **加 `custom` 前缀躲不开后缀模式。** `customDeptEndpoint` 照样以 `Endpoint` 结尾，改完再发一次会被拒同一条。后缀模式（`*_ENDPOINT`）要换掉**结尾**，前缀模式（`DATABASE_*`）才是加前缀——市场与 CLI 给的建议都按这条来。
...
# 6. 查看日志
docker compose -p brickkit-my-first-project logs -f hello-world-1-0-1
```
...
当项目中有很多组件源码时，你日常开发可能只关注其中 2~3 个组件。`brickkit sync` 会把这次不启动的组件源码归档到 `components/.archived/` 目录（判据与 `brickkit up` 完全一致）：
...
**核心规则：**
- `brickkit sync` 与 `brickkit up` 共用同一个启停判定
- 双向操作：该归档的归档，该激活的移回来
...
用 Git。`brickkit.yaml` 是要提交的文件，CLI 不再自建备份（003 §7.2 说明了为什么）：
...
`enabled` 有三种写法，规则是**跟着上层走**（003 §4.3）：

1. **不写 `enabled` 字段：跟着上层走。** 顶层（没有任何组件依赖它）默认跑；下层只要还有一个上层在跑就跑。强依赖和弱依赖一视同仁。
2. **`enabled: true`：一定跑**，不看上层。
...
注意：如果一个组件被 `enabled: false` 关掉，而某个**写了 `enabled: true`** 的组件强依赖它，CLI 会报错阻断——两个意图直接冲突。依赖方**没写** `enabled` 时不报错，它跟着一起不启动：那正是"我关掉了一整条链"的正常操作。
...
```bash
# Docker 环境
docker compose -p brickkit-my-first-project logs people-basic-1-0-0-migration
...
| 工作区管理         | `brickkit sync` 按启停判定结果双向归档/激活组件源码。归档目录为 `components/.archived/`                                 |
