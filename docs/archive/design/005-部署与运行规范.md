...
4. **都没写 `limits` 就不生成 `limits`**——平台不替人猜上限（见下）
...
`requests` 有默认值（`100m` / `128Mi`），**`limits` 没有**。组件与项目都没写时，生成物里连 `limits` 这一段都不出现。

理由是两个方向的失败代价完全不对称：

| | 后果 |
| --- | --- |
| **不设上限** | 组件用多少是多少。真出问题（内存泄漏）时节点变紧，kubelet 按 QoS 驱逐——运维看得见、查得出 |
| **平台猜一个上限** | 去 kill 一个跑得好好的组件：内存超了直接 `OOMKilled`、CPU 超了被 CFS 限流成 p99 毛刺，而组件作者从没同意过那个数字 |

从前默认给 `500m` / `512Mi`。那意味着**任何一个真的需要 600Mi 的组件，都会被一个平台编出来的数字反复 OOMKill**，而配置里一个字都没写过它。这与"平台提供工具，不替人做决定"（001 §12）直接冲突——**限额是业务判断，只有写下它的人知道那个数字对不对**。

`requests` 保留默认值，因为它的性质相反：它是**给调度器的提示**，不设会让 Pod 掉进 `BestEffort`（节点一紧第一个被驱逐），而给一个保守的小值只影响调度密度，不会杀任何东西。

#### 怎么设：CPU 与内存的建议相反

| | 建议 | 为什么 |
| --- | --- | --- |
| **CPU** | 设 `requests`，**不设 `limits`** | CPU 是可压缩资源：闲着的份额别人随便用，争抢时按 `requests` 权重分配，不会崩。而 CPU limit 走 CFS quota，**即使节点整体空闲**，进程也会在每个 100ms 周期里被限流，表现成毫无来由的 p99 毛刺 |
| **内存** | `requests` = `limits` | 内存不可压缩。两者相等即拿到 `Guaranteed` QoS，节点缺内存时**最后**才被驱逐；而 `requests` 远小于 `limits`（如 128Mi / 1Gi）意味着你告诉调度器只要 128Mi 却允许涨到 8 倍，一旦多个组件同时涨，你正是"超出 requests 最多"的那一类，**优先被驱逐** |

只写其中一半是合法的，而且往往正是想要的：
...
components:
  - id: people/basic
    ...
    resources:
      ...
      limits:   { memory: "1Gi" }        # 只压内存，CPU 不设上限
```

#### 一台机器能塞多少个组件

**`requests` 之和 ≤ 节点 allocatable** 是唯一的硬约束；`limits` 之和可以远超节点容量（超卖是 K8s 的正常用法）。

20 个组件按默认 `requests`（`100m` / `128Mi`）合计只要 **2 核 / 2.5G**——两台 4 核 8G 的机器绰绰有余。真正的成本不在配额，而在**每个进程的内存地板**，且几乎完全由语言决定：
...
最后一行是唯一会真的逼人改架构的数字。**在动手把组件合并成一个实例之前，先试这个顺序：**
调小堆 / 换 GraalVM Native Image（进程的内存地板掉到几十 MB，而**部署形态一点不变**）
→ 按需启用（012 §2.15：50 个组件的项目本地开发时可能只有 4 个容器在跑）
→ 都不够，才考虑合并。

合并是可行的，平台的地址注入天然支持它（调用方零修改），但**平台不提供任何支持**：
关卡清单、代价清单与两条不通的路（Scale-to-Zero、内存超卖）见仓库根《组件合并部署》，
不做的理由见 012 §2.21。

#### ⚠️ 内存 `requests` 报低会出事，CPU 报低不会

这是整节里最容易踩的一条，而且两种资源的后果**完全不同**。

**`requests` 是给调度器的承诺，不是给自己的配额。** 写 `50Mi` 不会让组件只用 50Mi——它照样用它需要的那么多。你只是让调度器**以为**它只要 50Mi，于是往一台机器上多塞了几个。

**CPU 报低基本无害。** CPU 是可压缩资源：不争抢时该用多少用多少，争抢时按 `requests` 作为权重按比例分配。报低的代价只是"抢不过别人"，慢，但不会死。

**内存报低会死人，因为内存不可压缩。** 节点物理内存耗尽时，kubelet 只能**杀掉一些 Pod** 来腾地方（这叫 eviction，与容器自己超 limit 被 OOMKill 是两回事）。而杀谁不是随机的：

| 顺序 | QoS 档 | 什么样的 Pod 属于这一档 |
| --- | --- | --- |
| 1️⃣ 最先杀 | `BestEffort` | requests 与 limits **都没设** |
| 2️⃣ 其次 | `Burstable` | 设了 requests，但与 limits 不完全相等 |
| 3️⃣ 最后杀 | `Guaranteed` | 每个容器的 CPU 与内存 **requests == limits** |

同一档内部，按**「实际用量超出自己 requests 多少」**排序，超得越多越先死。

于是完整的翻车剧本是这样：

```
你给 20 个组件都写了 requests: 50Mi（实际每个稳态 300Mi）
  → 调度器算出总共只要 1G，全塞进一台 8G 的机器
  → 跑起来实际吃 6G，加上系统预留，节点告急
  → kubelet 开始驱逐，先挑「超出 requests 最多」的
  → 而超得最多的，往往正是你**用得最多、最重要**的那个组件
  → 它被杀 → 重调度 → 还是这台机器 → 再被杀
```

**报低的组件不是"省下了资源"，是"排在了处刑队列的前面"。** 而且排最前的恰好是最忙的那个。

所以内存 `requests` 的正确含义是：**这个组件稳态下真的占这么多**，不是"我希望它只占这么多"。量出来再写——跑一周后 `kubectl top pods`，取观测到的稳态值（不是峰值，也不是理想值）。

> **不设配额（`BestEffort`）比报低更糟**：它排在处刑队列的第一位。这正是本平台保留 `requests` 默认值的原因——`limits` 可以不写，`requests` 不能没有。

#### 三个可以直接抄的场景

**① Go / Rust 组件群**——内存地板低，配额几乎不用操心：
...
components:
  - id: shop/order
    ...
    resources:
      ...
      limits:   { memory: "64Mi" }     # 相等 → Guaranteed；CPU 不设上限
```

20 个这样的组件合计 `1 核 / 1.3G`，两台 4 核 8G 的机器绰绰有余。

**② JVM 组件**——地板高，而且**堆外还要留一块**：
...
components:
  - id: erp/backend
    ...
    resources:
      ...
      limits:   { memory: "768Mi" }
    config:
      javaOpts: "-Xmx512m -XX:MaxMetaspaceSize=128m"   # 组件自己声明的配置项
```

⚠️ **`-Xmx` 必须明显小于内存 limit。** JVM 的常驻内存 = 堆（`-Xmx`）+ Metaspace + 线程栈 + 代码缓存 + 直接内存，堆外通常还要 **200–300 MB**。把 `-Xmx` 设成和 limit 一样大，容器会在堆还没满时就被 OOMKill——而 JVM 自己的日志里什么都看不到（进程是被内核杀的，不是 `OutOfMemoryError`）。**留 30–40% 给堆外**是常见起点。

**③ 突发型组件（报表、批处理）**——平时闲、偶尔猛：
...
components:
  - id: report/exporter
    version: 1.0.0
    resources:
      requests: { cpu: "100m", memory: "512Mi" }   # 按稳态写，好调度
      limits:   { memory: "2Gi" }                  # 允许峰值涨上去
```

这是**唯一**建议内存 `requests` ≠ `limits` 的场景：你明确知道它平时用不到那么多，愿意用"峰值时可能被驱逐"换"平时能多塞几个组件"。**不要把这种写法当默认**——对常驻服务它只会让你排在处刑队列前面。


💡 **同语言的组件用同一个 base 镜像**：同一镜像层的只读页（二进制正文段、共享库）在内核里是页缓存共享的，20 个容器跑同一个底座那部分物理内存只占一份。省的量不大但免费，顺带让拉镜像变快。
...
三种类型都会带上 `start_period`（启动宽限期，默认 60 秒，组件可用 `healthCheck.startPeriodSeconds` 覆盖，002 §9.3）——少了它，冷启动超过 30 秒的组件必然被判 unhealthy。
...
### 3.8 labels：透传给容器的部署元数据

组件的 `deployment.labels`（002 §4.7）与项目的 `components[].labels`（003 §4.11）
逐键合并后，原样写进组件 service 的 `labels`：

```yaml
services:
  erp-sales-1-0-0:
    image: registry.brickkit.io/erp-sales:1.0.0
    labels:
      traefik.enable: "true"
...
这一段的用途是让 Traefik / Caddy / Prometheus 这类工具用**官方镜像 + Docker
Provider 自动发现**接上来——平台不做网关（§5.11、012 §2.1），所以必须让人自己做。
带外部署的那个网关自己接进 `brickkit-<项目名>-net`（§3.6）就能看到这些标签。

**平台不解释键值，只透传。** 唯一的校验是拒绝覆盖平台自己拥有的键
（`app`、`brickkit.io/*`、`com.docker.compose.*`，见 002 §4.7）。

⚠️ **迁移容器上没有这一段。** 迁移不是服务，它跑完就退出——同一份标签写上去，
Traefik 会把它当成一个路由目标。这与「环境变量必须与主容器完全一致」（002 §8.5）
不冲突：那是因为迁移要连同一个库，而标签说的是「外面怎么找到这个服务」。

⚠️ **`local: true` 的组件上写了 labels 会收到警告**，因为它不生成容器
（§4.1），没有可以挂标签的对象。合并部署交付时这很常见——真正要挂标签的是
你自己写的那个外壳（《组件合并部署》§4.3）。
...
### 4.6.1 `expose` / `exposePort` 在 local 组件上不生效

`local: true` 不生成容器，平台因此没有端口可以映射到宿主机。那个进程已经在
宿主机上了，地址就是 `localhost:<localPort>`（§4.6）。

`up` 会警告一句，并把**真实地址**一起给出来——只说"不生效"不够，使用者正是
照着 `exposePort` 那个数字去开浏览器的：

```
⚠️ local: true 的组件上，expose / exposePort 本次不生效
   组件：demo/hello@1.0.0
   原因：它不生成容器（跑在你的 IDE 里），平台没有端口可以映射到宿主机
   写着的 exposePort：8888 —— 没有容器会去 bind 它
   实际地址：localhost:9999（由 localPort 决定，005 §4.6）
   建议：
   1. 让你的进程监听这个端口，浏览器直接访问它
   2. 要回到平台映射端口的模式，去掉这个组件的 local: true
```

这一条从前完全没守（003 §3.2 立的规矩是"写了不生效就得出声"）：配了
`exposePort: 8888` 的人打开浏览器什么都没有，而 `up` 全程一个字不说。
...
| 场景 | CLI 行为 |
| --- | --- |
| 依赖组件已有 `expose: true` + `exposePort` | **主端口**用已有的映射，不重复占一个；额外端口 expose 不管，照常单独映射 |
...
⚠️ **反过来，被依赖方的额外端口必须映射，`expose` 帮不上忙。** `expose: true`
只发布**主端口**（`<宿主机端口>:<deployment.port>` 那一行），额外端口一个都不在里面。
所以"依赖既 `expose` 又有 `extraPorts`"时，额外端口照常单独映射一个宿主机端口出来。

这一条曾经是错的：CLI 见到依赖已经 `expose` 就整个跳过映射，而 `local-debug.env`
里照样写着 `http://localhost:9090` —— 宿主机的 9090 根本没人监听。
表现是 **HTTP 通、gRPC 稳定 connection refused**，而两边的配置看上去都没毛病，
排查方向会一路跑到 gRPC 客户端和组件代码上去。

不变量是：**env 文件里写的每个 `localhost:<端口>`，compose 里都必须真有人把它发布出来。**
...
metadata:
  ...
  annotations:
    ...
    prometheus.io/scrape: "true"                 # ← 透传：labels 在 K8s 下落在这里
spec:
  replicas: 1
  selector:
    matchLabels:
  ...
  template:
    metadata:
      ...
      annotations:
        ...
        prometheus.io/scrape: "true"             # ← Pod 上也要有：抓取类注解读的是 Pod
    spec:
      containers:
        - name: people-basic
          image: registry.brickkit.io/people-basic:1.0.0
          ...
          env:
            ...
            - name: ENABLE_AUDIT
              value: "false"
          startupProbe:
            # 启动宽限期（002 §9.3）。它通过之前，下面两个探针都不执行；
            # 通过之后它自己不再运行。failureThreshold = ⌈startPeriodSeconds ÷ 5⌉
            httpGet:
              path: /healthz
              port: 8080
            periodSeconds: 5
...
#### 5.3.2 透传的 labels 在 K8s 下落在 annotations

`deployment.labels`（002 §4.7）与 `components[].labels`（003 §4.11）合并后，
在 K8s 目标下写进 **Deployment 与 Pod 模板的 `annotations`**，而不是 labels。

上一节（§5.3.1）已经说了一半原因：**label 的「值」只允许 `[A-Za-z0-9._-]`。**
而要透传的值里正是那些字符——`` PathPrefix(`/erp/sales`) `` 有斜杠、反引号与括号，
写进 labels 会被 API Server 整份拒绝，报的还是那句看不出所以然的
`a valid label must consist of alphanumeric characters...`。

另一半原因是**会打架**：这份清单里的 `app: <版本化服务名>` 是 Deployment 找到
自己 Pod 的唯一依据，也是 NetworkPolicy 的匹配依据（§5.13.1）。透传键进了
labels，一旦撞上就是选择器选空或策略放行错对象——**两者都不报错**。
所以平台干脆不给这个机会：`app`、`brickkit.io/*`、`com.docker.compose.*`
在校验阶段就被拒绝（002 §4.7）。

**Pod 模板上也写一份**，不是冗余：`prometheus.io/scrape` 这一类抓的是 Pod，
只写在 Deployment 上等于没写。

**Service / Ingress / 迁移 Job / PDB / NetworkPolicy 都不带这些注解。**
Ingress 已经有自己的注解口（`deploy.ingressAnnotations`，003 §3），两个口子写
同一处会互相覆盖而且没人说得清谁赢；迁移 Job 跑完就退出，被当成抓取目标是 bug。

⚠️ **改了 labels 会触发一次滚动重启。** Pod 模板的注解变了，模板哈希就变了——
这是 K8s 的正常行为，也是想要的行为（新注解得让新 Pod 带上）。
...
上一段那句"只要 `hostname` 不同就不冲突"反过来也成立：**`hostname` 相同就是冲突，
而且是生成阶段的硬错误。**

每条规则都是 `host` + `path: /`。两个组件共用一个 `hostname`，就是两条一模一样的
规则指向不同后端——K8s 对此没有定义行为（nginx-ingress 取创建时间最早的那份并记
一条冲突日志）。表现是外面打进来的请求稳定落到其中一个上，而生成、`apply`、
`kubectl get ingress` 全都看不出任何问题。

```
❌ 错误：域名 shop.example.com 被多个组件占用
   组件：demo/caller@1.0.0
   组件：demo/hello@1.0.0
   原因：每个 expose 的组件都生成一条 host + path: / 的 Ingress 规则；两条一模一样的规则指向不同后端，K8s 没有定义行为——请求会稳定落到其中一个上，而 apply 不会有任何抱怨
   建议：
   1. 在 brickkit.yaml 中给其中一个组件换一个 hostname（一个组件一个子域名）
   2. 或去掉其中一个组件的 expose: true（组件之间在集群内互访不需要 expose）
```

**为什么是报错不是警告：** 与 Docker 侧对称。那边两个组件抢同一个宿主机端口
同样是硬错误（§10.2），两条出路的形状也一样——给其中一个换个值。一个
"生成成功、apply 成功、路由随机"的部署，比一次生成期的失败难查得多。

**多版本共存时几乎必然踩到：** 照 003 §8.3 加第二个版本时，整个组件条目是复制
出来的，`hostname` 跟着一起复制。这里也没有"两个版本轮流服务"这种解释——
两份 Ingress 不是负载均衡。要按版本分流请给它们各自的域名
（`v1.shop.example.com` / `v2.shop.example.com`），或者只暴露其中一个。

**没有 `expose: true` 的组件即使写了 `hostname` 也不占域名**——它根本不生成
Ingress（那个字段会在 `up` 时被"写了不生效"的提醒点名，003 §3.2）。

> **平台不做 path 路由**（`example.com/api` → A、`example.com/` → B）。那是一个新
> 字段、新语义，而 K8s 的常规做法本来就是一个组件一个子域名。真要按路径分流，
> 那是集群侧的路由策略——自己写一份 Ingress 就好，平台只负责"把这个组件按域名
> 暴露出去"这一件事。
...
**但测不出来不等于不说。** 生成了 NetworkPolicy 时，`brickkit up` 每次都会打印：

```
🔒 已生成 5 份 NetworkPolicy（deploy.networkPolicy.enabled: true）
   ...
   平台测不出来（K8s 没有这个 API），只能你自己验一次：
      按 005 §5.13.0 真的连一次，看该通的通、该断的断
```

这一段从前只在本节与 003 §3.2 里——而打开这个开关的人多半是从附录 D 抄了个
字段，不会回头读这两节。于是完整的失败路径是：抄字段 → `up` 静默生成 →
`apply` 成功 → `get networkpolicy` 看得见 → **以为收紧了，实际全通**。
...
漏声明是**真 `up` 的阻断项**——出站一旦生效，没声明的资源一律连不上（见上）。
但 `--dry-run` 的语义是"告诉我会发生什么"（§004 §4.4），硬拦的后果是：一个正在配
egress 的人连"看看会生成什么策略"都做不到，而那恰恰是他最需要看的东西。

```
⚠️ 警告：出站策略没覆盖全部资源（--dry-run 不阻断）
   缺少声明的资源：pg-main（demo/hello 要用它）
   ...
   4. 不加 --dry-run 时这里会直接阻断——出站一旦生效，没声明的资源一律连不上
📄 已生成 5 份清单：.brickkit/generated/k8s/
```

这与资源绑定检查是同一条规则（006 §4.4）：**"该不该阻断"是命令层的决定**，
生成器只负责说清楚是什么问题。
...
#### 6.2.1 同一组件的多个版本，迁移按版本号串行

多版本共存（003 §8.3）时，同一组件的每个版本各有一个迁移容器。它们**必须一个
跑完再跑下一个**，CLI 生成的 compose 里因此多一条链：

```yaml
demo-hello-1-0-0-migration:
  # 版本最低的那个不等任何人

demo-hello-2-0-0-migration:
  depends_on:
    demo-hello-1-0-0-migration:
      condition: service_completed_successfully   # ← 等上一个版本的迁移成功结束
```

**为什么必须串。** 资源绑定按**组件 ID** 记（不带版本，003 §5.3），所以同一组件的
多个版本拿到的 `DATABASE_NAME` 必然是同一个；迁移状态表的主键是
`(component_id, version)`（002 §8.11），两个版本的 `component_id` 也是同一个。
于是"两个迁移容器同时对同一个库、用同一个身份跑迁移"完全是**平台自己生成出来的**
——使用者在 `brickkit.yaml` 里只是照 003 §8.3 写了两行版本号。

迁移只增不改（002 §8.10），所以高版本的迁移集合是低版本的**超集**。这在老库上
没问题：低版本发现那批迁移已应用就跳过、干净退出。但在**空库**上两个容器都会去
跑重合的那部分，一个成功、另一个撞主键退出，那个版本的主服务永远停在 `Created`。

> **它只在空库上撞，而且重跑一次就好**（那时已经写进去了），错误信息指向数据库
> 主键冲突，与"我配了多版本"看不出任何关系。间歇性 + 自愈 + 报错指向别处，
> 是最费时间的那一类——所以由平台一次性消掉，而不是留给使用者去撞。

**只串同一个组件 ID。** 不同组件的 `component_id` 不同，主键
`(component_id, version)` 已经让它们互不相干——那正是 002 §8.11 设计这个主键的
目的。把它们也串起来只会平白拖慢 `up`（迁移是所有组件的前置阻塞步骤）。

**顺序按版本号，不是服务名的字典序。** 后者会把 `10.0.0` 排在 `2.0.0` 前面，
于是先跑的是更新的那一版。

⚠️ **这条链只作用于这一次 `up`。** 两个人同时 `brickkit up` 打同一个开发库、
...
**⚠️ 不是"全部 apply 再逐个 wait"。** Job 一 `apply` 就开始跑，那种写法等于让所有
迁移同时开跑，`wait` 只是在旁边看着——同一组件的多个版本会去抢同一个库（理由与
Docker 侧完全一样，见 §6.2.1）。CLI 实际是按**组**走的：

| 维度 | 行为 |
| --- | --- |
| 组的划分 | 按**组件 ID**：同一个组件的所有版本是一组，按版本号升序 |
| 组内 | 严格串行：`apply` 一个 → `wait` 一个 → 再下发下一个 |
| 组间 | 并行：每一轮先把各组的当前 Job 都下发出去，再逐个等 |

不同组件之间不串，理由见 §6.2.1（主键里的 `component_id` 已经让它们互不相干，
串起来只会拖慢 `up`）。
...
| **同一组件的多个版本串行** | 按版本号升序，一个跑完再跑下一个（两种目标都是）。不同组件之间并行。见 §6.2.1 |
...
**「最长依赖链」是这一屏里唯一与 `up` 时长有关的数。** 六个组件，关键路径只有
4 层——`infra/redis-event-bus` 是弱依赖，不约束顺序，它与整条链并行启动。

> 这里从前写的是"必须最后启动：X（需等前 N 个组件就绪）"，N 取的是**拓扑序号
> 减一**。序号只是把整张图压平成的一条直线，它会把毫不相干的另一条链上的组件
> 也算进"要等的前 N 个"——读者据此以为整个 `up` 是串行的，组件一多就会以为
> 启动线性变慢。同一段里另外两行（`可独立启动：1`、`本地调试：people-basic`）
> 与 `（本地调试模式）` 这个后缀，CLI 也从来没打印过。
