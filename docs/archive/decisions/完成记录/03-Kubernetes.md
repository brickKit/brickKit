> ⚠️ **历史存档，可能与当前 CLI 行为不一致。** 这是 brickKit 重构文档体系前的旧版原文，不再维护——理解现在的平台请看 <https://github.com/brickKit/brickKit/tree/main/docs/zh/architecture> 与 <https://github.com/brickKit/brickKit/tree/main/docs/zh/guide>。
...
### 实现 P37：出站策略 — ✅ 2026-08-16

**为什么它和入站不是一回事：** NetworkPolicy 的语义是，一个 Pod 只要在
**Egress 方向**被任何策略选中，该方向**未明确允许的一律拒绝**。
所以生成第一条出站规则的那一刻，组件就从"想连谁连谁"翻转成"只能连白名单"。
难点不在生成规则，而在**漏一项就出事**。
...
| 目标 | 谁负责 | 理由 |
| --- | --- | --- |
| DNS | 平台自动放行 | 谁都要，漏了什么都不通。而 kube-dns 的位置各集群不一样，逼人查一次再抄就是制造必踩的坑 |
| 组件依赖 | 从依赖图推导 | 与入站是同一张图的两面。让人再写一遍必然过期 |
| 资源位置 | 使用者声明 | 平台知道谁绑了哪个，就是不知道它在集群哪儿 |
| 资源端口 | 平台从 `resources[].port` 取 | 配置里已经有了，抄第二遍早晚不一致 |

**声明不全就阻断**是核心：平台从 `bindings` 知道哪些组件绑了哪些资源，
少一个就在生成阶段报错，绝不生成一份会让组件连不上库的策略。

**真跑验证（calico 集群，department/tree + 另一个命名空间里的 postgres）：**

| 场景 | 结果 |
| --- | --- |
| 故意不声明资源位置 | 阻断，点名"pg（department/tree 要用它）"，并给出可直接粘贴的配置 |
| 补全后部署 | 起来了，业务请求跨命名空间连库返回真实数据 |
| 手工删掉 postgres 那条规则 | **业务照常 200**（连接池里的连接还在） |
| 删规则后重启 Pod | 组件**起不来**：8080 connection refused、日志为空、就绪探针一直失败 |
| 恢复规则再重启 | 回到 200 |

**⚠️ 实测修正了我原先的判断。** 写代码时我在注释和设计书里断言："漏了数据库的话
组件照常启动、healthz 也过、业务全挂——部署全绿业务全挂"。
真跑下来 **department/tree 是起不来**，因为它启动时就建连。正确的说法是
**后果取决于组件什么时候建连**：启动建连的起不来（显眼），
首次请求才建连的才是"healthz 绿、业务挂"。已把三处注释、配置文档与设计书全部改正。

**另一个真发现，比原判断更阴险：改策略不会杀掉已建立的连接。** 上表第三行——
规则删了、业务照常 200。问题要等到**下一次重启**（节点排空、升级、扩缩容）才暴露，
那时离改配置可能已经过去几周，现场没有任何线索指向那次改动。
这条已写进设计书与测试注释。
...
| # | 决策 | 理由 |
| --- | --- | --- |
| D407 | 出站单独一个 `egress.enabled`，不复用 networkPolicy 的开关 | 它会翻转默认行为（从"想连谁连谁"到"只能连白名单"）。跟入站共用一个开关，等于让打开入站策略的人在不知情的情况下也承担了出站的全部风险 |
| D408 | DNS 由平台自动放行，且放行**集群内任意命名空间**的 53 | 漏了 DNS 什么都不通，而 kube-dns 的位置各集群不一。让步很小：`namespaceSelector: {}` 只匹配集群内 Pod，出不了集群。UDP/TCP 都放——大响应会退回 TCP，只放 UDP 的表现是"偶尔解析失败" |
| D409 | 资源端口从 `resources[].port` 取，写了 `resource` 就禁止再写 `ports` | 两处各写一份早晚不一致，而不一致的表现是"策略看着对、组件连不上库"。禁止比容忍更省事 |
| D410 | 资源声明不全**在生成阶段阻断** | 平台有能力知道（bindings 里写着）。放过去的后果取决于组件建连时机，两种都不好查，而且已经跑着的实例还不受影响——问题会推迟到下次重启 |
| D411 | 资源类目标只加给**真的绑了它**的组件 | 给用不到那个库的组件也开口子，等于白白放宽范围。绑定关系配置里就有，没有理由不用 |

---

### 修复 P36：网络策略挡掉了监控 — ✅ 2026-08-16

**缺陷：** 生成的策略只放行依赖图里的组件，而 Prometheus 不在那张图上。
组件就算声明了 `observability.metrics: true`，抓取照样被挡——
**现象是指标悄悄停了**：没有报错、没有 5xx，只是图表上的线断了。
这是最难查的一类故障，也是 P26 交付时就登记下来的真缺口。

**修法：** 新增 `deploy.networkPolicy.allowFrom`，遵循平台已有的分工线——
能推导的由平台推导（依赖图），推导不出的由使用者声明**意图**，
而不是让他贴一段 NetworkPolicy YAML。`ingressController` 是这条线上的第一个实例。

```yaml
allowFrom:
  - name: prometheus            # 必填，写进 brickkit.io/allow-from 注解
...
| 场景 | 结果 |
| --- | --- |
| monitoring 命名空间 + 正确标签 | **HTTP 200** |
| monitoring 命名空间但标签不对（grafana） | **超时**（exit 28） |
| 标签对但在别的命名空间 | **超时** |

**后两条同时被拦，才是这次最有价值的验证。** 它证明命名空间与标签是 AND：
两个 selector 若写成两个 `from` 元素就变成 OR，那时后两条都会通——
而且 apply 成功、流量也通，不会有任何迹象说明放行范围比预期大。
D384 那条判断第一次得到了实证。

另做 A/B 对照：去掉 `allowFrom` 后第一条立刻回到超时，加回来又是 200——
证明这个修复确实改变了行为，而不是本来就通。

**顺手的重构：** `ingressControllerSource` 与 `allowFromRules` 形状相同、
踩的是同一个 AND/OR 坑，抽成 `namespacedSource` 一个出口——
那条语义现在只有一处，写错的机会从两处降到一处。

**文档：** 005 §5.13.1.1 从"已知限制"改写成"怎么声明"（→ 1.17.0），
§5.13.5 的后续方向里移除已实现的 allowFrom（只剩 allowTo / P37）；
试用指南 18.2 从"已知缺口 + 绕过办法"改写成解法，并带上那个三路实验；
README 的已知缺陷表清空。
...
| # | 决策 | 理由 |
| --- | --- | --- |
| D403 | `allowFrom` 加到**每一个**组件的策略上，不做按组件区分 | 监控要抓的是全部组件。漏掉任何一个的表现都是"那个组件的指标没了"而它本身好好的，比一次性配全麻烦得多。真需要只对某个组件开口子时再说 |
| D404 | 不写 `ports` 时放行组件**声明过**的全部端口 | 使用者多半不知道每个组件的端口是几——那本来就是组件自己声明的。逼他抄一遍既啰嗦又容易过期：组件加了 extraPorts 就得回来改配置 |
| D405 | `name` 必填，并写进 `brickkit.io/allow-from` 注解 | 它不是装饰。半年后有人 `kubectl get networkpolicy -o yaml` 看到一条放行某命名空间的规则，得能立刻知道是为谁开的——否则只能在"不敢删"里躺着 |
| D406 | 两处来源共用 `namespacedSource` 一个出口 | namespaceSelector 与 podSelector 的 AND/OR 是整份策略里最容易写错、且写错完全无声的地方。留一个出口，写错的机会就只有一处 |

---

### 修复 P38：K8s 升级后旧版本变成孤儿 — ✅ 2026-08-16

**缺陷：** `kubectl apply` 只创建和更新，**不会删**目录里没有的资源。
于是把版本号从 1.0.0 改成 2.0.0 再 `up`，1.0.0 的全套资源继续跑着——
而 `status` 只报本次部署的、`down` 也只删生成目录里有的，构成**永久泄漏**。
Docker 那条路由 compose 的 `--remove-orphans` 兜底，K8s 这边一直是空白。

**修法：** `up` 在 rollout 就绪后查一次集群实际资源，与本次期望集合求差，删掉多余的。

四条规则，每条都对应一个真实风险：

| 规则 | 不这么做会怎样 |
| --- | --- |
| 在 **rollout 就绪之后**才清理 | 新版本还没起来就删了旧的，中间一段谁都服务不了——升级从"无感知"变成"短暂 502" |
| 按 `brickkit.io/project=<项目名>` 收窄 | 很多组织把多个项目放同一个命名空间，不收窄会删到别人的服务 |
| **`--only` 时完全不清理** | 那时只部署子集，没点名的组件照样在服务——删它们是**把线上组件下线**，比留个孤儿严重得多 |
| 删了什么**必须打印** | 使用者可能只是误删了配置里的一行，本意并非下线那个组件；这几行是他唯一的线索 |

清理范围刻意**不含 Namespace**（它上面也有项目标签，算进去的话一次普通升级
就会把整个命名空间端掉）与 Secret（按资源 ID 命名，与服务名集合对不上）。

**真跑验证（minikube）：**

| 场景 | 结果 |
| --- | --- |
| demo/hello 1.0.0 → 2.0.0 再 `up` | `🧹 已清理旧版本残留（2 项）`，逐条列出 |
| `kubectl get deployment` | **只剩 2.0.0**（修复前这里是两个） |
| **多版本共存 + `--only demo/hello@2.0.0`** | 在跑的 1.0.0 **没被删**——最危险那条路验过了 |

最后一行是这次最要紧的验证。`--only` 下 `plan.services` 只是子集，
照着它清理会把没点名的组件全删掉；这条路只在真集群上、
且必须造出"有组件在跑但没被点名"的场景才验得了。

**测试：** 引擎层 10 条（命令怎么拼、顺序、边界），命令层 5 条（该不该清、清了说没说）。
其中 `TestUpK8sDoesNotPruneWithOnly` 与 `TestKubectlUpNeverPrunesCurrentVersion`
守的是"改坏了会把线上组件删掉"这条线。

**文档：** 005 新增 §5.9.1（→ 1.16.0）；试用指南 8.5 从"已知缺陷 + 手工清理命令"
改写成"自动清理"，README 的已知缺陷表移除 P38。

**决策记录：**

| # | 决策 | 理由 |
| --- | --- | --- |
| D398 | 清理放在 rollout 就绪**之后** | 先删后起会让升级出现一段无实例可服务的窗口。多留一会儿孤儿没有代价，早删一秒就是 502 |
| D399 | `--only` 时**完全不清理**，用"命令层不给选择器"表达 | 空选择器让引擎直接返回，逻辑只有一处判断。若改成传标志位再在引擎里判，就有两个地方可能写反，而写反的后果是删掉正在服务的组件 |
| D400 | 清理范围不含 Namespace 与 Secret | Namespace 上也有项目标签，算进去一次普通升级就会端掉整个命名空间；Secret 按资源 ID 命名，与"本次服务名集合"对不上，会被误判成孤儿 |
| D401 | 查询失败只警告，不让整次 `up` 失败 | 部署已经成功，清理只是收尾。因为查不到集群状态就判失败，会让人以为服务没起来而去做多余的回滚 |
| D402 | 删掉的资源逐条打印，且带类型前缀 | 悄悄删东西不可接受。带前缀（`deployment.apps/xxx`）是因为使用者要拿它直接去 kubectl 里查证 |
...
**试用指南：** 新增 [18-网络策略与最小权限.md](../../guide/18-网络策略与最小权限.md)，
...
同时把"目前已实现的每一项能力怎么用"整理成 [试用指南/](../../guide/)，可以按编号一节一节跟着走。
...
**试用指南（[试用指南/](../../guide/)）：** 11 份文档 + 一个 `准备.sh`。每节结构统一为
