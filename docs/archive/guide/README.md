...
每一篇末尾都有一条 **为什么是这样 →**，指向
[012 架构设计原理与考量](../design/012-架构设计原理与考量.md) 里对应的那一节。

那是一份**架构辩护书**：二十节，每一节回答一个"为什么不那样做"——
为什么只收精确版本、为什么 `remove` 连源码一起删、为什么弱依赖缺不到时
连环境变量都不注入、为什么 50 个组件不会失控。

**这些不是背景知识。** 每一条都直接决定你会不会误用，或者在踩到之后
以为是 bug 而去查代码。做完一节觉得"这行为有点怪"的时候，先翻那一节。

> `make check-docs` 会核对 012 的二十节**每一节都有指南指过去**——
> 新写一节辩护而没人引，等于写完就沉底。
