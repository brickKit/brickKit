...
前面每一篇装的都是本来就已经存在的东西。这一篇真写一个全新的组件——一个最小的 HTTP 计数器，故意小到能完整抄下来——然后用这个系列里其它每一篇一模一样的方式把它跑起来：没有专门的上手路径，没有脚手架命令，就是一份 `component.yaml` 加一个镜像。
...
- [从市场发布与安装](08-marketplace.md)和[给组件签名与验签](09-signing.md)也是同一套方式——`brickkit publish --path ./components/tutorial/counter`，签名，装到别的地方去。
