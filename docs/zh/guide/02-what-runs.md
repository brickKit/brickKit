...
这个项目只是一条直线似的两组件链条。真实的依赖图里会有菱形（同一个组件被不止一条路径依赖），循环依赖有时候也是合法的——[依赖解析与启动顺序](../architecture/dependency-resolution.md)用真实例子把这两种都讲透了，还讲了为什么八个组件启动不等于八步串行。下一篇：[本地调试一个组件](03-local-debugging.md)——这两个组件里的一个挂着断点跑在你自己的机器上，另一个照常运行，感觉不到对面到底是不是一个容器。
