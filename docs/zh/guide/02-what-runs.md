...
这个项目只是一条直线似的两组件链条。真实的依赖图里会有菱形（同一个组件被不止一条路径依赖），循环依赖有时候也是合法的——[依赖解析与启动顺序](../architecture/dependency-resolution.md)用真实例子把这两种都讲透了，还讲了为什么八个组件启动不等于八步串行。这个系列的下一篇（完整规划见[教程索引](README.md)）：给这些组件里的一个挂上断点做本地调试，同时让其它组件照常在容器里跑着。
