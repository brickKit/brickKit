...
│    → 各部各的。零成本，而且通常更好
├─ 组件自己无状态，状态在它连的资源里
│    → 各部各的组件 + 指向同一个资源 —— 最常见的情况
└─ 组件本身就是权威（单例副作用，或者跑两份代价高昂）
     → 把它当成别人的 API 来调
```
...
这是当前这个系列的最后一篇（还规划了哪些内容见[教程索引](README.md)）。这个系列动手走过的任何一个具体机制，想深挖细节，`docs/zh/architecture/` 和 `docs/zh/patterns/` 里都有配着真实代码和真实生成输出的完整讲解——AGENTS.zh.md §11.2 有完整的索引。
