...
同一条路上还会拦下：配置项名撞上平台保留变量（警告）；`configSchema.required` 里的键既没有默认值、也没被项目覆盖（**阻断**——`--dry-run` 也一样以退出码 1 拒绝往下走）。完整的报错文案见[环境变量注入契约](../06-architecture/04-environment-variables.md)第五节。
...
种子数据与测试数据该怎么规划——哪些数据该固化进迁移脚本、哪些该在测试运行时现场构造——是独立的一个问题，见[怎么规划种子数据与测试数据](02-data-construction.md)。
