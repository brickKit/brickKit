...
「市场」这一行说的是角色，不是一个现成的地址：BrickKit 目前没有官方运营的公开市场实例。一个项目要用上市场，要么自己搭一套，要么用别人已经搭好给你用的那一套（AGENTS.zh.md §5.9）。
...
| [18. 代平台向外部密钥存储取值](#18-代平台向外部密钥存储取值)<br>CLI 替你调用密钥管理器 SDK 取值、写成 Secret | 每接一种存储就多一个 SDK 和一份凭据，每次 `up`（含 `--dry-run`）都要联网 | 自己把值放进环境；已经存在的 Secret 直接用 `existingSecret` 引用 |
...
| [19. 引擎插件与第三方部署目标](#19-引擎插件与第三方部署目标)<br>一个可插拔接口，让第三方把部署交给平台不认识的地方 | 插件要自己担保拆卸和状态查询，而 CLI 会替它报"成功"——当初撤掉 Podman 就是这个教训，后来它自己的引擎在仓库内重建了才回来（第 16 条） | 新目标在仓库内实现，带全套测试守卫 |
...
- **替代做法：** 每个环境一份完整、自包含的 `brickkit.yaml`，用 `brickkit up --config brickkit.prod.yaml` 选择。想看两份环境配置到底有什么不同、这些不同会带来什么后果，看[怎么选部署形态](../07-patterns/05-deployment-selection-guide.md)里那份两步配方。
...
- **它是什么：** CLI 在 `up` 时调用 Vault / AWS Secrets Manager 之类的 SDK，把取到的值变成 Secret。
- **为什么不做：** 每接一种存储就多一个 SDK 和一份凭据，每次 `up`（含 `--dry-run`）都要联网取值，这跟被否决的配置中心是近邻；而 `${VAR}` 本来就已经先查进程环境。
- **替代做法：** 用你自己的工具把值放进环境，再 `brickkit up`；在 `configSchema` 里声明 `secret: true`，让配置里的凭据不出现在 Deployment 里；对于外部系统已经建好的 Secret，直接用 `resources[].existingSecret` 或者 `secret: true` 配置项的 `{ existingSecret, key }` 写法引用它——见[密钥](../07-patterns/10-secrets.md)。

---

### 19. 引擎插件与第三方部署目标

- **它是什么：** 一个供外部程序实现的接口，让类似 `up` 上 `--engine nomad` 这样的参数，把部署交给平台不认识的地方。
- **为什么不做：** 一个目标的拆卸、状态查询、孤儿清理保证，正是"一个项目能被彻底拆干净"这件事成立的原因；插件会自己担保这些，而 CLI 却替它报"成功"——当初撤掉 Podman 就是这个理由，后来它自己的引擎在仓库内重建了才回来（第 16 条）。用命令行参数选目标，也会绕开 `deploy.target` 这条声明本身。
- **替代做法：** 在仓库内实现新目标，带上全套测试守卫。

---

### 20. 增量生成缓存

- **它是什么：** 在 `.brickkit/` 下记住哈希状态，让 `up` 只重新生成变化的部分。
- **为什么不做：** 50 个组件走完整条生成链路只要约 2 ms；使用者真正在等的是 `docker compose up` / `kubectl apply`，它们本来就只动有变化的部分；而缓存一旦过期，会静默产出错误的部署文件。
- **替代做法：** 什么都不做——没有什么好加速的。先测量（`go test ./tests/perf -bench .`）；只有真实项目的生成时间超过大约 100 ms，才值得重新考虑。

---

### 21. 生成 mock 与自动替换缺失依赖
...
- **它是什么：** 让 `brickkit graph` 自己产出 HTML 页面或 SVG 图片、自带一个查看器，或者长出一个把输出写进文件的参数——而不只是打印 Mermaid 文本。
- **为什么不做：** Mermaid 文本本来就有人免费渲染：GitHub 直接渲染 `.mmd` / `.mermaid` 文件，以及 Markdown 里标了 `mermaid` 的代码块，什么都不用装。CLI 里再内置一个渲染器，是给一件今天不花钱的事添一份永久的维护成本——排版、多一种要保证正确的输出格式。写文件的参数也用不着：stdout 里只有 Mermaid（提示是 `%%` 注释行，警告走 stderr），普通的 shell 重定向存下来的就是一个合法文件。
- **替代做法：** `brickkit graph > graph.mmd`，再到 GitHub 上打开那个文件；或者把输出贴进 Markdown 里标了 `mermaid` 的代码块。

---

### 23. 在 lint 里做依赖与跨文件检查

- **它是什么：** 让 `brickkit lint` 去解析依赖图、检查跨文件的引用——`servedBy` 指向的组件在不在项目里，强依赖找不找得到。
- **为什么不做：** `lint` 是一个承诺：离线、只读、秒回，不需要 Docker 或 Kubernetes，也没有自己的规则——它跑的就是 `up`、`add`、`publish` 本来就对每个文件做的解析加校验。解析依赖图要读每个组件的 Manifest，对市场 / Git 组件就意味着联网；只要联一次网，这个承诺就没了。
- **替代做法：** `brickkit up --dry-run`——它无论如何都要解析依赖图，`servedBy` 目标不存在、或者有解析不出来的强依赖时，会报错并点出是哪一个；想看声明出来的结构，用 `brickkit graph`。（`lint` 同样不拿 `configSchema` 里自己声明的 `enum`、`minimum` 去检查配置的值——见[第 9 条](#9-配置值的类型校验)。）
