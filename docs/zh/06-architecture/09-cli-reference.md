...
只有警告不会让运行失败。把 `configSchema` 里 `greeting` 那一项的 `default` 拼成 `defualt`，`brickkit lint` 会打印下面直到 `📋` 那一行为止的内容，然后退出 `0`。加上 `--strict`，同样的内容之后还多一块——stderr 上的汇总错误——并退出 `1`。下面是加了 `--strict` 的那次运行：
