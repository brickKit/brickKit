...
CLI 的报错文案跟着语言走——默认英文，`brickkit lang set zh` 之后是中文（见 [`brickkit lang`](09-cli-reference.md#brickkit-lang)）。这一篇引用的是中文原文，[英文版](../../en/06-architecture/10-error-codes.md)引用英文原文；不管哪一种，你都可以直接拿终端里打印的那句话来搜。错误**码**本身永远不随语言变——这正是脚本可以放心依赖它的原因。
