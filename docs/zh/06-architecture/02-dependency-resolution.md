...
下面每一个例子都是对着 [`tests/components/`](../../../tests/components/) 里真实存在的组件跑出来的真实 `brickkit up --dry-run` 结果，不是手写的示意图。CLI 默认说英文，下面引用的是它的中文输出（`brickkit lang set zh` 切换），跟你自己在中文下跑一遍看到的一字不差。
