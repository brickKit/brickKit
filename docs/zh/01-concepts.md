...
「市场」这一行里"可自己部署一套"才是重点：BrickKit 目前没有官方运营的公开市场实例。一个项目要用上市场，要么自己搭一套，要么用别人已经搭好给你用的那一套（AGENTS.zh.md §5.9）。
...
| 本地调试模式 | Local Debug Mode | `mode: debug`：让一个组件跑在你自己的电脑上（比如在 IDE 里打断点调试），容器里的其它组件照样能找到它 |
| 本地托管模式 | Local Managed Mode | `mode: local`：BrickKit 自己探测怎么启动这个组件，把它跑成你自己电脑上的一个裸进程，并且自己盯着它——不用你操心，也不用开 IDE |
...
| 写法 | 含义 | 行为 |
| --- | --- | --- |
| **不写** `mode` 字段 | 跟着上层走 | 顶层组件（没人依赖它）默认跑；下层组件只要还有上游需要它，就跟着跑 |
| `mode: enabled` | 一定跑 | 不看上层。如果它的强依赖被关掉了，直接报错（两个互相矛盾的意图） |
| `mode: disable` | 一定不跑 | 依赖它的组件跟着一起不跑 |
| `mode: debug` | 一定跑，进程由你自己启动 | 跟 `mode: enabled` 一样钉住，只是不生成容器：你在自己的机器上、IDE 里跑它（仅限 Docker，见[第 3 篇](03-guide/03-local-debugging.md)） |
| `mode: local` | 一定跑，进程由 BrickKit 自己启动并监管 | 跟 `mode: enabled` 一样钉住，只是不生成容器：BrickKit 自己探测启动命令、拉起它、盯着它——不用开 IDE（仅限 Docker，见[第 4 篇](03-guide/04-local-execution.md)） |

被多个上游共享的下层组件，只要至少有一个上游还在跑，它就不会被误关掉。这也是为什么 `brickkit add` 自动写入的依赖从不带 `mode` 字段——留白就是"跟着上层走"。
