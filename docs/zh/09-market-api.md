...
- [签名与信任模型](06-architecture/06-signing-and-trust.md) —— 市场存签名但不验签名，为什么这么设计
- [自己搭一套 BrickKit Market](07-patterns/09-deployment/self-hosted-market.md) —— 部署市场本身
- [CLI 命令完整参考](06-architecture/09-cli-reference.md) —— `login`/`publish`/`add` 等命令怎么用这套 API
