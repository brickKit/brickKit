...
| 症状 | 一句话原因 | 怎么解 |
| --- | --- | --- |
| [20. `up` 警告 `mode: local` 组件还没监听，明明它已经打出看着像启动成功的输出](#20-up-警告-mode-local-组件还没监听明明它已经打出看着像启动成功的输出) | 组件没读注入的 `PORT` 变量，实际监听的端口跟 BrickKit 在探测的那个不是同一个 | 让组件自己的启动代码从环境变量读 `PORT`；或者把 `localPort` 固定成它实际写死的那个端口 |
| [21. `brickkit down` 或者另一个终端按 `Ctrl+C` 都停不掉它](#21-brickkit-down-或者另一个终端按-ctrlc-都停不掉它) | `mode: local` 进程只属于跑 `up` 的那一个终端，别的地方都够不到它 | 去那个终端按 `Ctrl+C`；`status`/`down`/`graph` 都会点名那个会话的 PID |
...
### 20. `up` 警告 `mode: local` 组件还没监听，明明它已经打出看着像启动成功的输出

- **症状：** `<服务名> 还没监听端口 <N>——可能只是启动得慢`，紧跟在组件自己的输出后面打出来——而那段输出看着完全正常，不像报错。
- **原因：** BrickKit 往进程的环境变量里注入了 `PORT`（默认自动分配，或者用 `localPort` 固定），并且最多探测这个端口 30 秒。如果组件自己的代码没读 `PORT`——比如写死了 `:8080`——它照样正常启动、正常监听，只是监听的不是 BrickKit 在盯着的那个端口。这条警告真的是非致命的：`up` 不会杀掉这个进程，也不会停下来，组件在它实际绑定的那个端口上照样能连上。
- **解决：** 让组件启动时从环境变量读 `PORT`，跟它在 `mode: debug` 下或者在容器里的读法一样。如果现在没法改代码，就把 `localPort` 写成它实际写死的那个端口，让 BrickKit 探测对地方。

---

### 21. `brickkit down` 或者另一个终端按 `Ctrl+C` 都停不掉它

- **症状：** `brickkit down` 之后，或者在跑 `up` 的那个终端之外的**另一个**终端按 `Ctrl+C`，`mode: local` 组件照样在跑。
- **原因：** 这是设计如此，不是 bug。`mode: local` 进程只被跑 `brickkit up` 的那一个终端会话监管——它没有容器，所以 `down`（或任何别的命令）没有引擎可以去请求它停下，不像这个项目里其它任何东西。
- **解决：** 去真正在跑 `up` 的那个终端按 `Ctrl+C`。在别的地方跑 `status`、`down`、`graph`，都会打印一句点名那个会话 PID 的提示（`这个项目有一个本地会话在跑（PID <n>）——去那个终端看，或者在那边 Ctrl+C`）——那是唯一能停掉这个进程的地方。
