...
`mode: debug`（[第 3 篇](03-local-debugging.md)）是"你自己启动这个进程——在 IDE 里、带着断点——BrickKit 只负责把地址指过去"。`mode: local` 是同一个"裸进程、不生成容器"想法的另外半边：BrickKit 自己探测怎么启动这个组件、自己拉起它、自己盯着它——不用开 IDE，不用打断点，什么都不用你手动跑。这一篇把这条链路真实走一遍：`demo/hello` 作为一个 BrickKit 自己拉起来的普通操作系统进程在跑，另一个终端看得见它，停掉它的方式跟停掉任何一个前台命令一样。

## 设置

一个全新的项目，一个组件，真实的 Go 源码——不预编译二进制，不写 Dockerfile，就一份 `go.mod` 加一份读 `PORT` 监听端口的 `main.go`：
...
`brickkit new demo/hello` 生成骨架 `component.yaml`，`brickkit add --local` 把它写进 `brickkit.yaml`，一个字段就能让它变成一个由 BrickKit 本地托管的组件——跟 `mode: debug` 一模一样的写法，只是值不同：
...
```
🚀 启动项目 hello-local（deploy.target: docker）
📋 组件状态计算：
   ✅ demo/hello@0.1.0  启动（顶层）
...
demo-hello-0-1-0  已监听端口 8080
```

这次不会返回——`up` 会一直待在前台，跟不加 `-d` 的 `docker compose up` 一样，把进程自己的输出实时打出来，每一行都带着服务名前缀。第一行 `demo-hello-0-1-0 |` 是 `main.go` 里那句 `fmt.Printf` 原样转发出来的；第二行没带竖线前缀，是 BrickKit 自己打的：它一直探测 8080 端口，直到探到有东西真的在那边应答——跟一个容器的健康检查做的是同一件事。
...
💡 这个项目有一个本地会话在跑（PID 975891）——去那个终端看，或者在那边 Ctrl+C
```
...
```
graph TD
    demo_hello_0_1_0["demo/hello@0.1.0<br/>托管本地"]
...
```
{"time":"...","level":"INFO","message":"命令执行完成","command":"brickkit up","elapsed_ms":22452,"exit_code":0}
```

没有额外的"正在关闭"提示，也没有崩溃报告——因为什么都没崩。`demo/hello` 被要求停下，停下了，`up` 以 `0` 退出。这份安静是故意的：崩溃摘要专门用来暴露**意料之外**的退出（下一节），一个完全照要求停下的组件如果也打一份出来，就只是噪音。`mode: local` 组件的进程树也是故意只属于这一个终端会话的——它从不试图活得比启动它的那次 `up` 更久（不像容器，CLI 退出后容器照样接着跑），所以事后没有任何要清理的东西，也没有类似 `docker ps -a` 那种"已停止但还留着"的条目。
