...
派生其余一切——启动顺序、服务地址、环境变量、Docker Compose 或 Kubernetes 清单、
网络策略——然后交给 Docker 或 Kubernetes，自己退出。没有注册中心，没有配置中心，
没有网关，没有常驻进程。

每个组件是一个独立的领域单元，独立开发、独立测试、独立部署、独立调用。整个设计
借用的，都是工程师本来就信任的想法，并且它工作的单位足够小，人或 AI 一次就能读完。
...
| --- | --- | --- |
| **限界上下文**（DDD）<br>把系统按业务领域切成几块，每块自己管自己 | 组件是独立单元：自己的仓库、Manifest、版本生命周期和契约 | 任何时候只需要理解一个组件 |
| **声明式期望状态**<br>只写"要什么"，不写"怎么做" | `brickkit.yaml` 是唯一输入，放在 Git 里，每个环境一份完整文件。CLI 用完即走，没有控制面 | 描述目标，而不是部署脚本；diff 就是评审 |
| **派生优于配置**<br>能算出来的就让工具算，不让你去写 | 启动顺序、服务地址、`*_ENDPOINT` 变量、Compose/Kubernetes 清单、网络策略，全部由依赖图算出来 | 派生值不会与来源漂移，也没有人需要去猜变量名或端口 |
| **十二要素配置**<br>配置不写进代码，启动时用环境变量传进去 | 地址、资源连接、配置都以环境变量到达；Docker 与 Kubernetes 上是同一种地址格式 | 组件代码不知道自己跑在哪——换环境零改动 |
| **精确版本，并排共存**<br>依赖写死具体版本，不写"1.x 都行" | 不接受范围；版本号是服务名的一部分（`people-basic-1-0-0`） | 两个版本作为两个 DNS 名字共存，AI 写的 v2 可以和 v1 并排跑，不动 v1 的调用方 |
| **契约先行**<br>先把接口写成文件，再各自实现 | `artifacts` 随 Manifest 携带 API 契约；市场要求提供 API 的闭源组件必须声明它 | 不读代码也能读懂一个组件的边界 |
| **大声失败**<br>出错就立刻明显地失败，不悄悄带着错继续 | 未知的 Manifest 键直接拒绝；缺失的弱依赖**什么都不注入**（绝不注入空字符串）；写错的 config 键会警告 | 错误在 `up` 或启动时暴露，而不是变成生产里一个悄悄的错误答案 |
| **最小权限**<br>只给完成工作所需的最少访问，其余默认不通 | 声明之前什么都不可达；可选的网络策略由依赖图生成；cosign 签名的组件只用 Go 标准库验证 | 默认更小的爆炸半径，信任锚在**你自己的**项目里 |

每个想法是什么、有什么好处和代价、BrickKit 怎么对待它——包括 BrickKit 刻意留给你的
那些，比如六边形架构和 TDD——用大白话写在
[设计原则与取舍](docs/zh/architecture/design-principles.md#认识这些想法)里。
