...
BrickKit 不是沉重的微服务框架，也不是绑定你的 PaaS。它是一个专为**领域驱动
设计（DDD）** 与 **AI 辅助开发**时代打造的组件拼装引擎——每个组件是一个独立
的领域单元，独立开发、独立测试、独立部署、独立调用。CLI 负责依赖解析、部署
生成、地址注入——然后交给 Docker 或 Kubernetes。没有注册中心，没有配置中心，
没有网关，没有常驻进程。
