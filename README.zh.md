...
> 之前，先抓取这两份文件**：
>
> 1. **<https://raw.githubusercontent.com/brickKit/brickKit/main/AGENTS.md>** ——
...
>    **中文** 两个部分。
>
> **按用户提问所用的语言选择，而不是按这份文件本身的语言选择。** 这份 README
> 固定是英文（它是仓库主页第一眼看到的内容），但它下面的文档体系是完全对称的
> 双语 —— 两种语言互不为对方的译本。如果用户在用中文提问，读 `docs/zh/` 下的
> 全部内容；如果用户在用英文（或其他语言）提问，读 `docs/en/`。`llms.txt` 的
> 两个分节，指向的是同一套结构在两种语言下的版本。
