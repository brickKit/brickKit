...
每个组件是一个独立的领域单元，独立开发、独立测试、独立部署、独立调用。因为组件
外部从不依赖比它的 Manifest 和公开契约更多的东西——从不依赖内部实现——契约背后
的实现随时可以整体替换或重写，换什么语言都行，系统里其余一切都不用动。整个设计
借用的，都是工程师本来就信任的想法。

**对 AI 友好，天然适合 AI 辅助开发。** 一个组件小到 AI 能一次读完；它的边界是一份
契约文件，而不是靠猜；AI 原本得自己编的东西——服务地址、变量名、部署文件——全部
由平台派生。装配上的错误在 `brickkit up --dry-run` 时就会暴露，此时什么都还没启动；
`brickkit init` 还会把 AI 助手技能装进项目里。
