...
| 平台为什么长成这样：贯穿一切的那个想法（声明一张图，其余派生）；它用到或刻意没用的每个工程想法（DDD、GitOps、十二要素、契约先行、六边形架构、TDD……），逐个编号介绍：从"它是什么"讲起，说清好处、代价、对应 AI 开发的什么痛点、BrickKit 怎么具备、为什么刻意不具备（留白）、AI 怎么适应这块留白；十二条原则各自的论证 | `docs/zh/architecture/design-principles.md`（英文版把 `zh` 换 `en`） |
