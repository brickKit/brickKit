...
| 平台为什么长成这样：贯穿一切的那个想法（声明一张图，其余派生）；它用到或刻意没用的每个工程想法（DDD、GitOps、十二要素、契约先行、六边形架构、TDD……），从"它是什么"讲起，说清好处、代价，以及 BrickKit 怎么对待它、为什么；十二条原则各自的论证 | `docs/zh/architecture/design-principles.md`（英文版把 `zh` 换 `en`） |
