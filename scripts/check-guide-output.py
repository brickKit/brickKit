...
⚠️ **2026-09-19 起移出 `make lint`，待决定去留。** 下面 CASES 里的每一条，
不管是裸文件名（落到 `docs/archive/guide/`）还是显式的
`docs/archive/design/...`，核对的对象**只有归档文档**。而归档现在明确
不再要求与 CLI 保持同步——清理 CLI 错误文案里指向已归档设计书的引用
（AGENTS.md/README 同一轮清理）之后，这个脚本立刻报出一批"对不上"：
不是文档过期了没人发现，是归档文档**从今往后本来就不会再改**，而 CLI
文案还会继续演进，两边只会越漂越远。继续把它拉进 `make lint` 等于逼着
每一次 CLI 文案调整都要回头去改一份"历史记录、不维护"的文档，这与归档
本身的定位矛盾。

真正的缺口："现行"的 `docs/{en,zh}/guide/`（12 篇，取代了这里守的 23 篇
归档指南）反而**完全没有**这种逐行输出核对——那 12 篇里同样有真实的
CLI 输出块，值得同一套机制去核对，只是目前没人接上。两个选项，谁都没做：
① 整个脚本重写一遍，CASES 改成对着 `docs/{en,zh}/guide/` 的锚点；
② 承认它已经历史完成使命，连同这份脚本一起退役。

在这两者选一个之前，脚本本身、下面的 CASES、`prepare`/`disable`/`pin`
等夹具函数都原样保留——机制是好的，只是对错了目标，删掉可惜。
