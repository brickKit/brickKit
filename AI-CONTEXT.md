...
>
> 全站文档为中文，术语请沿用本文的中文说法（组件 / 强依赖 / 版本化服务名 / 启停判定……）。
...
```
brickkit.yaml（声明）
   ↓ ① 启停判定：算出这次实际该启动哪些组件（enabled + --only）
...
| 启停判定 | Enablement | 配置里有它、又没写 `enabled: false`，就启动；强弱依赖一视同仁 |
...
| 写法 | 行为 |
| --- | --- |
| **不写** `enabled` 字段 | ✅ 启动（`brickkit add` 写进来的就是这个样子） |
| `enabled: true` | ✅ 启动（**与不写完全等价**） |
| `enabled: false` | ❌ 不启动 |

**规则只有一句：配置里有它、又没写 `enabled: false`，就启动。**
强依赖与弱依赖一视同仁——`optional: true` 管的是"解析期取不到算不算错"
与"它没跑时注不注入 `*_ENDPOINT`"，不管"跑不跑"。

**关掉一个组件不会连累别人。** 只有它自己不跑；若有正在启动的组件**强依赖**它，
`up` **报错**并点名依赖方（要一起关就给它们也写 `enabled: false`）。

想让本地只跑几个组件，用 `brickkit up --only <组件>`——它取依赖闭包（强弱都算），
**不改配置**。这就是为什么"一个 50 组件的项目，本地开发时可能只有 4 个容器在跑"。

> 早先这里是三态 + 级联（关掉一个，依赖链上无人需要的自动跳过）。已删除，
> 理由见 012 §2.14：那是平台替使用者做的隐式决定，而它换来的东西 `--only` 做得更好。
...
| `brickkit sync` | **双向**归档 / 激活：不启动的移到 `components/.archived/`，需要启动的移回。`--only` 按依赖闭包收拢，不改配置 |
...
components:
  - id: people/basic
    ...
    enabled: true                # 可选，只有 false 有实际效果，见 5.4
...
| `brickkit status` | 读底层引擎，展示运行表格（含多版本检测、未启动组件及原因） |
| `brickkit sync` | 双向归档 / 激活组件源码；`--only` 按依赖闭包收拢 |
...
**9.14 为什么 `enabled` 只有开和关，没有"级联"？**
早先关掉一个组件时，依赖链上无人需要的组件会自动跟着跳过。删掉了：那是平台替使用者
做的一次**隐式**决定，与"brickkit.yaml 就是声明"（写了就启动）直接冲突——
最刺眼的后果是弱依赖"写进配置却不启动"。想收窄这次的范围用 `up --only`：
显式说要哪几个，更精确，也不动团队共享的配置。完整论证见 012 §2.14。
