...
func (s *Service) CreateOrganization(
	...
	if id == nil || id.Anonymous {
		return nil, model.Errorf(model.CodeUnauthorized, "creating an organization requires logging in")
	}

	name := strings.TrimSpace(req.Name)
	if name == "" {
		return nil, model.Errorf(model.CodeInvalidRequest, "organization name must not be empty")
	}
	// 一个人只能属于一个组织（users.org_id 是单值）。已经在别的组织里时明确报错，
	// 不能悄悄把他挪走——那会让他原组织的 private 组件突然读不到了
	if id.OrgID != "" {
		return nil, model.Errorf(model.CodeConflict,
			"you already belong to organization "+id.OrgID+"; a user can only belong to one organization")
	}
...
func (s *Service) ListOrganizations(ctx context.Context, id *Identity) ([]model.Organization, error) {
	if id == nil || id.Anonymous {
		return nil, model.Errorf(model.CodeUnauthorized, "querying organizations requires logging in")
	}
...
func (s *Service) AddOrganizationMember(
	...
	if id == nil || id.Anonymous {
		return model.Errorf(model.CodeUnauthorized, "adding a member requires logging in")
	}

	org, err := s.repo.GetOrganization(ctx, orgID)
	if err != nil {
		if errors.Is(err, repo.ErrNotFound) {
			return model.Errorf(model.CodeNotFound, "organization not found: "+orgID)
		}
		return internalError(err)
	}
	if !id.IsAdmin && org.OwnerID != id.UserID {
		return model.Errorf(model.CodeForbidden, "only the organization owner or a Market admin can add members")
	}
	...
	default:
		return model.Errorf(model.CodeConflict,
			username+" already belongs to organization "+user.OrgID+"; remove them from it first")
	}
